...
1060316	Leslie	Kevin_C	1050087		[航港532]修正組織樹沒有BaseTable導致Template取BaseTable的Top值時出錯
...
1120428	Leslie	Cloud	1120211		修改texterea enter 不跳控制項
*/
...
document.addEventListener( "DOMContentLoaded",function(){
	$('.V3_GenericBannerToolBar > input[type=submit] , .V3_GenericBannerToolBar > select').each(function () { 
	...
	//1060316	Kevin_C	1050087		[航港532]修正組織樹沒有BaseTable導致Template取BaseTable的Top值時出錯
	//if($('.DivBaseTable,.BaseTable').offset().top < $('#tbTool').height())$('.DivBaseTable,.BaseTable').css('margin-top',$('#tbTool').height()+20+'px');
	if($('.DivBaseTable,.BaseTable').length && $('.DivBaseTable,.BaseTable').offset().top < $('#tbTool').height())
		$('.DivBaseTable,.BaseTable').css('margin-top',$('#tbTool').height()+20+'px');
	
	//1110701 Kevin 1110629 新增ToolBar分隔線
	$('<span class="ToolBarSpan">|</span>').insertBefore(  $('.V3_GenericBannerToolBar input:not([style*="display: none"]):not(:first):not(.hide)') );
...
function jf_CheckDataExist(argOrgNo)
{
	...
	//1101014	Leslie[1090482]	停用STD，改為STDN
...
function jf_ConvertEnterToTab(event)
{
	...
	if(event.returnValue == false)
		return;
	//1120428		Cloud	1120211		修改texterea enter 不跳控制項
...
function jf_GetArtifact()
{
	if (document.all.SsoArtifact)
		...
		return encodeURI(document.all.SsoArtifact.value);
	return "";
}

//1050113	Leslie	二代公文系統修改(配合行動平台，改為傳入按鈕ID)
//function jf_ToolBarSubmit()
function jf_ToolBarSubmit(argSubmitBut)
{
	...
	//1061019 Cloud 移至確定postback 再紀錄
...
//1050504	Leslie	二代公文系統修改
//function jf_SelectBarSubmit()
function jf_SelectBarSubmit(argSubmitBut)
{
...
function jf_CheckSpecialKeyPress()
{
	if(event.ctrlKey)
	{
		if(event.keyCode == "123")
		{
			...
			{
				//1071127	Leslie	升級改用二代專用頁面
				//jf_OpenMsgWin("../../../STD/LIB/MsgDiag.aspx", "MsgDiag");
				jf_OpenMsgWin("../../../STDN/LIB/MsgDiag.aspx", "MsgDiag");
			}
		}
	}
}
...
function Server(){
	...
	function ensureWsdlUrl(szService, bCORS){	//1051117	Leslie	新增可支援跨站台呼叫(不強制轉為同網域網址)
		//增加代換為目前同一DomainName的URL
		if (szService.indexOf("://") > 0){
			var pageDomain = document.location.origin;
			if(!bCORS)	//1051117	Leslie	新增可支援跨站台呼叫(不強制轉為同網域網址)
	...
	//1091117	Leslie[1080943]	改用WebMethodInfo取代WSDL
	//this.useService = function (argService,UserName, bCORS){	//1051117	Leslie	新增可支援跨站台呼叫(不強制轉為同網域網址)
	this.useService = function (argService,UserName, argFunName, bCORS){	//1051117	Leslie	新增可支援跨站台呼叫(不強制轉為同網域網址)
		if(argService == null || argService.length == 0){
			throw("Invalid arguments");
			return;
		}	
		
		var url = ensureWsdlUrl(argService, bCORS);	//1051117	Leslie	新增可支援跨站台呼叫(不強制轉為同網域網址)
		//1091117	Leslie[1080943]	改用WebMethodInfo取代WSDL
...
function jf_CallW(argService, argFunName, argAsync, argParam , bCORS)	//1051117	Leslie	新增可支援跨站台呼叫(不強制轉為同網域網址)
{
	...
	//1091117	Leslie[1080943]	改用WebMethodInfo取代WSDL
	//service.useService(argService,"Serv", bCORS);	//1051117	Leslie	新增可支援跨站台呼叫(不強制轉為同網域網址)
	service.useService(argService,"Serv", argFunName, bCORS);	//1051117	Leslie	新增可支援跨站台呼叫(不強制轉為同網域網址)
	return service.Serv.callService(argFunName,argParam,argAsync);
}

function jf_CallWS(argService, argFunName, argAsync, argParam, bCORS)	//1051117	Leslie	新增可支援跨站台呼叫(不強制轉為同網域網址)
{
	...
	return jf_CallW(argService, argFunName, argAsync, arPara, bCORS);	//1051117	Leslie	新增可支援跨站台呼叫(不強制轉為同網域網址)
}

function jf_CallWA(argService, argFunName, argAsync, argParam, bCORS)	//1051117	Leslie	新增可支援跨站台呼叫(不強制轉為同網域網址)
{
	return jf_CallWS(argService, argFunName, argAsync, argParam, bCORS);	//1051117	Leslie	新增可支援跨站台呼叫(不強制轉為同網域網址)
}