...
1131009	Kevin   Cloud   1130941         弱掃修改
1140401 Kevin   Zen     1140556         配合滲透測試調整至Server判斷圖形驗證
*/
/*****************************************************************************
*
*   Declaration 區
...
var theForm = document.forms['DLI100'];
if (!theForm)
{
    theForm = document.DLI100;
}
...
function ClientOnLoad()
{
    ...
    document.all.DLI100.onsubmit = function () { return WebForm_OnSubmit() };
    //1130818	Cloud	1130747	新增驗證碼子視窗
    $("a[id$='hltbFileVerify']").click(function (e)
    {
        fnShowVerifyInfp(e.target.id);
    });
...
function btSearchss()
{
    ...
    if (document.all['btOutVerify'].className != 'hide')
    {
        ...
        //1140401 Zen 1140556 配合滲透測試調整至Server判斷圖形驗證
        //else
        //{
        //    //1110928 Leslie  增修因AjaxPro無法通過資安設定環境下的替代方案，全都改成WebService處理
        //    //var result = DL1.DLI100.VerifyCaptcha(txOutVerify, objCaptcha.value.strKey);
        //    //1140401 Zen 1140556 配合滲透測試調整至Server判斷圖形驗證
...
function CheckSDate()
{
    ...
    //1131009	 Cloud   1130941         弱掃修改
    //return CheckToolbarDate(oSDate, "發文日期(起)", true);
    if (!CheckToolbarDate(oSDate, "發文日期(起)", true))
        $('#txStartIssueDate').focus();
}
...
function CheckEDate()
{
    ...
    if (!CheckToolbarDate(oEDate, "發文日期(迄)", true))
        $('#txEndIssueDate').focus();
}
...
{
    //1110928 Leslie  增修因AjaxPro無法通過資安設定環境下的替代方案，全都改成WebService處理
    //objCaptcha = DL1.DLI100.GenCaptcha();
    var params = new SOAPClientParameters();
    ...
    //1140401 Zen 1140556 配合滲透測試調整至Server判斷圖形驗證
    document.all['H_txVerifyKey'].value = objCaptcha.value.strKey;
}
//1130815   Cloud   1130747         調整DG介面
function fnShowVerifyInfp(argShaInfo)
{
    //將資訊寫入LOCALSTOAGE
    var strVerifValue = argShaInfo.split('_')[0] + "__" + argShaInfo.split('_')[2] + "_tbFileHash";
    var strDocNo = argShaInfo.split('_')[0] + "__" + argShaInfo.split('_')[2] + "_tbdgIssueNo";
    localStorage.setItem(document.all["H_SOURCEORGNO"].value + "-" + document.all[strDocNo].textContent, document.all[strVerifValue].textContent);
    var strUrl = "DLI100C1.aspx?DocNo=" + document.all[strDocNo].textContent + "&OrgNo=" + document.all["H_SOURCEORGNO"].value;
    jf_OpenChildWin(strUrl, "DLM100C1", screen.width, screen.height * 0.8);
}