 ...
 * 1140122  Zen         Zen         1121108     支援產生正式公文PDF
 */
/*****************************************************************************
*
*   Declaration 區
...
function DownLoadFileToClient(argMode)
{
    ...
    //1140122 Zen 1121108 支援產生正式公文PDF，整合至下方
    //var strFileName = "";
    ...
    */
    ...
    //1140122 Zen 1121108 支援產生正式公文PDF
    var strFileName = "";
    if (argMode == "1" || argMode == "2")
        strFileName = document.all.H_lbDi.textContent;
    else
    ...
    var callObj = TB1.TBI140.GetFile(document.all.H_Artifact.value, document.all.lbBulletinId.textContent, strFileName, argMode, u_lockedUrl, u_lockedPath, H_txDocNo.value, H_lbAllowName.textContent).value
    if (callObj[0] != "")
    {
        alert("取得檔案下載失敗，錯誤：" + callObj[0]);
        return;
    ...
    //1140122 Zen 1121108 支援產生正式公文PDF
    //jf_OpenChildWin(callObj[1], "");
    window.open(callObj[1]);

    //1051004   Cloud   1050087 升級二代--E
}
...
//1140122 Zen 1121108 支援產生正式公文PDF，取得轉檔工作站
var u_serverList = new ServerList();
var u_lockedUrl = "";
var u_lockedPath = "";

function ServerList()
{
    var _list = document.all.H_ImgConvertUrl.value.split(';');
...
function doQS()
{
    ...
    {
        var params = {};
        var url = u_suffle.pick();
        if (url)
        {
            _url = url;
            var params = new SOAPClientParameters();
            params.add('Url', url);
            //var r = SOAPClient.invokeJSON(jf_Trim(document.all.H_txENWS.value), "QueryState", params, false, null)
            ...
            if (typeof r.value === "string" && r.value.length > 0)
            {
                ...
                if (p > 0)
                {
                    ...
                    {
                        u_lockedUrl = url;
                        u_lockedPath = r.value.substring(p + 1);
