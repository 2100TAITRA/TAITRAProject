...
function jf_Check()
{
	...
	if (document.all["txEndDate"].value == "")
	{
		strErrMsg += "日期迄值不可空白\n";
        //1050929	Kenny   [1050087]	二代公文系統相關修改
	...
	{
		bRtnbool = false;
		jf_ShowMsg( FormatStr( jf_GetErrMsg(CustErr), new Array(["日期起訖欄位不可空白"]) ), "" );
	}
...
function fnCalculateDays()
{
	...
	if (strSDate != "" && !jf_CheckCDATE(strSDate))
	{
        ...
        $('#txStartDate').focus();
        jf_ShowMsg( FormatStr( jf_GetErrMsg(InFormatErr2), new Array(["日期起值格式有誤"]) ), "" );
        //1050929	Kenny   [1050087]	二代公文系統相關修改--End--
		return false;
	}
	if (strEDate != "" && !jf_CheckCDATE(strEDate))
	{
		...
		//jf_ShowMsg( FormatStr( jf_GetErrMsg(InFormatErr2), new Array(["日期格式有誤"]) ), "" );
        $('#txEndDate').focus();
        jf_ShowMsg( FormatStr( jf_GetErrMsg(InFormatErr2), new Array(["日期迄值格式有誤"]) ), "" );
        //1050929	Kenny   [1050087]	二代公文系統相關修改--End--
		return false;
	}
}
