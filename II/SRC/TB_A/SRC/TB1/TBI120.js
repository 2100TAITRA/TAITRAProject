...
1120802	Leslie	Zen		1120465	(中興大學)支援以子視窗顯示公告對象
1131230 Zen     Jason   屏東序1223 修改1升2變數錯誤宣告問題
1140205 Zen     Jason   屏東序1223 修改網址參數多傳入判斷是否為進階查詢
1140630 Zen		Daniel	1140299	新增查詢過期公文功能。
*/
/*****************************************************************************
*
*   Declaration 區
...
{
	//1031107	Cloud	-----	-------	4.5net觸發postback時會觸發ClientButtonControl，此時document.activetElement可能為null 會導致異常
	//1100201	Joe		1090927		取消使用document.activeElement，移除非二代行為之無用判斷
...
function ReturnValue(argBulletinId, argSeqNo,argSearchExpire)
{
	//fnOpenBulletin(argBulletinId, argSeqNo);
	fnOpenBulletin(argBulletinId, argSeqNo, argSearchExpire);
}
...
function fnSetData(argPasteDateStart, argPasteDateEnd, argDocNo, argBulletinId, argUnitCode, argCategory, argSubject, argRank, argShowNoRead, argRemoveProxyRead, argAccount, argRpsDeptNo, argSortWay, strSearchExpire)
{
	...
	//1140630	Daniel 1140299	新增查詢過期公文功能。
	document.all["txSearchExpire"].value = strSearchExpire;
}
...
function fnPageNumOnChange()
{
	...
	//1131230    Jason   屏東序1223 修改1升2變數錯誤宣告問題
...
function fnOpenBulletin(argBulletinId, argSeqNo,argSearchExpire)
{
...
//	var argArtifact = parent.fnGetArtifact();
	//1140205     Jason   屏東序1223 修改網址參數多傳入判斷是否為進階查詢
	var txIsAdvanceSearch = "";
	if (document.all["txIsAdvanceSearch"].value == "" || document.all["txIsAdvanceSearch"].value == "N")
		txIsAdvanceSearch = "N"
	else
		txIsAdvanceSearch = "Y"
	...
	//1140630	Daniel	1140299	新增查詢過期公文功能。
	//strUrl = "TBI130.htm" + strParam + "&rtnObj=lbReturnValue&BulletinId=" + argBulletinId + "&SeqNo=" + BulletinNowNum + "&BulletinNum=" + BulletinNum + "&IsAdvanceSearch=" + txIsAdvanceSearch;
	strUrl = "TBI130.htm" + strParam + "&rtnObj=lbReturnValue&BulletinId=" + argBulletinId + "&SeqNo=" + BulletinNowNum + "&BulletinNum=" + BulletinNum + "&IsAdvanceSearch=" + txIsAdvanceSearch + "&SearchExpire=" + argSearchExpire;
...
//1120802 Zen 1120465 (中興大學)支援以子視窗顯示公告對象
function OpenTBI121(argBulletinId)
{
	let strUrl = 'TBI121.aspx?argBulletinId=' + argBulletinId;
	jf_OpenChildWin(strUrl, 'TBI121', 1024, 768);
}
...
function CallDownLoad(argBulletinId,argUserName, argOrgNo)
{
	...
	//1140630 	Daniel	1140299	修正點擊下載後會發生錯誤。
	//var rtn = TB1.TBI120.DownLoadZip(argBulletinId, argUserName, argOrgNo);
	var rtn = TB1.TBI120.DownLoadZip(argBulletinId, argUserName, argOrgNo, document.all.txIsAdvanceSearch.textContent, document.all["txSearchExpire"].value);
    if (rtn.value.strErr != "")
