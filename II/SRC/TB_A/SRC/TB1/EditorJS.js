...
1060831 Kevin   Zen     1060742     弱點掃描修正
1090311	Kevin	Joe		1080166		弱掃修正Client Potential XSS
*/
//  EDITOR PUBLIC (API)
...
function appendHTML(szHTML)
{
	...
	idEditbox.document.body.contentEditable = "True";//IE 5.5以上才支援
}
...
function setMode(bMode) 
{
	if (bMode!=g_state.bMode) {
		...
		if ((bMode) && (g_state.bMode)) 
		{
			...
			//1090311	Joe		1080166		弱掃修正Client Potential XSS
			// objBody.innerHTML = idEditbox.document.body.innerText
			objBody.innerHTML = HtmlEncode(idEditbox.document.body.innerText)
		}
		_setSize()
...
function _Format(szHow, szValue)
{
	...
	case "InsertHorizontalRule":
		insertHTML("<IMG SRC=\"" + L_EMOTICONPATH_TEXT + "greypixel.gif" + "\" WIDTH=\"100%\" HEIGHT=\"2\" VSPACE=\"9\">");
		break;
	case "Justify":
		if (oBlock) 
		{
			...
			if (oBlock.tagName=="IMG" && szValue!="Center")
				break;
		}
		szHow=szHow+szValue
		szValue=""
...
function _initEditor() {
	...
	+   "</STYLE>"
	+   "<BODY style=\"" + L_DEFAULT_BODY_STYLE + "\" ONCONTEXTMENU=\"return false\">"
	+		"<DIV></DIV>"
	+   "</BODY>"
	_CPopup_Init()
...
{
	//1020523	Cloud	[1020088] 因段落按鈕實際功能非字樣所顯示可調整段落，將此功能取消，避免使用者誤用
	//var aSizes	= new Array(25			,25			,25			,7	,54				,48			,71				,7	,25			,25			,25			,8	,25				,25				,25				,8	,25				,25					,25			,25			,8	,25					,25			,25			,25				,8	,25				,25				,8	,25);
	//var aIds	= new Array("cut"		,"copy"			,"paste"		,"bar1"	,"formatblock"			,"fontstyle"		,"fontsize"			,"bar2"	,"bold"			,"italic"		,"underline"		,"bar3"	,"left"				,"center"			,"right"			,"bar4"	,"orderedlist"			,"unorderedlist"			,"outdent"		,"indent"		,"bar5"	,"line"					,"link"			,"table"		,"image"			,"bar6"	,""				,"textcolor"			,"bar7"	,"emoticon");
	//var aTips	= new Array(L_TIPCUT_TEXT	,L_TIPCOPY_TEXT		,L_TIPPASTE_TEXT	,""	,L_TIPP_TEXT			,L_TIPFSTYLE_TEXT	,L_TIPFSIZE_TEXT		,""	,L_TIPB_TEXT		,L_TIPI_TEXT		,L_TIPU_TEXT		,""	,L_TIPLJ_TEXT			,L_TIPCJ_TEXT			,L_TIPRJ_TEXT			,""	,L_TIPOL_TEXT			,L_TIPUL_TEXT				,L_TIPDINDENT_TEXT	,L_TIPIINDENT_TEXT	,""	,L_TIPLINE_TEXT				,L_TIPLINK_TEXT		,L_TIPTABLE_TEXT	,""				,""	,""				,L_TIPFGCOLOR_TEXT		,""	,L_TIPEMOTICON_TEXT);
	...
	//var aDisplay	= new Array("block"		,"block"		,"block"		,"block","block"			,"block"		,"block"			,"block","block"		,"block"		,"block"		,"block","block"			,"block"			,"block"			,"block","block"			,"block"				,"block"		,"block"		,"block","block"				,"block"		,"block"		,"none"				,"block","none"				,"block"			,"none"	,"none");
	var aSizes		= new Array(25					,25					,25					,7			,48						,71							,7		,25					,25						,25							,8				,25								,25								,25								,8			,25								,25									,25						,25					,8			,25									,25						,25						,25							,8		,25								,25								,8		,25					);
	var aIds		= new Array("cut"				,"copy"				,"paste"			,"bar1"		,"fontstyle"			,"fontsize"					,"bar2"	,"bold"				,"italic"				,"underline"				,"bar3"			,"left"							,"center"						,"right"						,"bar4"		,"orderedlist"					,"unorderedlist"					,"outdent"				,"indent"			,"bar5"		,"line"								,"link"					,"table"				,"image"					,"bar6"	,""								,"textcolor"					,"bar7"	,"emoticon");
	var aTips		= new Array(L_TIPCUT_TEXT		,L_TIPCOPY_TEXT		,L_TIPPASTE_TEXT	,""			,L_TIPFSTYLE_TEXT		,L_TIPFSIZE_TEXT			,""		,L_TIPB_TEXT		,L_TIPI_TEXT			,L_TIPU_TEXT				,""				,L_TIPLJ_TEXT					,L_TIPCJ_TEXT					,L_TIPRJ_TEXT					,""			,L_TIPOL_TEXT					,L_TIPUL_TEXT						,L_TIPDINDENT_TEXT		,L_TIPIINDENT_TEXT	,""			,L_TIPLINE_TEXT						,L_TIPLINK_TEXT			,L_TIPTABLE_TEXT		,""							,""		,""								,L_TIPFGCOLOR_TEXT				,""		,L_TIPEMOTICON_TEXT);
	var aCommand	= new Array("_Format('cut')"	,"_Format('copy')"	,"_Format('paste')"	,null		,"_CPopup_Show('font')"	,"_CPopup_Show('fontsize')"	,null	,"_Format('bold')"	,"_Format('italic')"	,"_Format('underline')"		,null			,"_Format('Justify','Left')"	,"_Format('Justify','Center')"	,"_Format('Justify','Right')"	,null		,"_Format('insertorderedlist')"	,"_Format('insertunorderedlist')"	,"_Format('outdent')"	,"_Format('indent')",null		,"_Format('InsertHorizontalRule')"	,"_CPopup_Show('Link')"	,"_CPopup_Show('Table')","_CPopup_Show('Image')"	,null	,"_CPopup_Show('BackColor')"	,"_CPopup_Show('ForeColor')"	,null	,"_CPopup_Show('Emoticon')");
	var aDisplay	= new Array("block"				,"block"			,"block"			,"block"	,"block"				,"block"					,"block","block"			,"block"				,"block"					,"block"		,"block"						,"block"						,"block"						,"block"	,"block"						,"block"							,"block"				,"block"			,"block"	,"block"							,"block"				,"block"				,"none"						,"block","none"							,"block"						,"none"	,"none");
...
var sz = "<TABLE style=\"display:none\" CELLSPACING=0 CELLPADDING=0 ID=idMode>"
...
insertHTML('<INPUT CLASS=DataBound SIZE=' + HtmlEncode(iLen + 2) + ' NAME="' + HtmlEncode(sElemName) +'" VALUE=" ' + HtmlEncode(sElemName) + ' ">')
...
{
sz +=  "<IMG TABINDEX=" + (i+1) + " ONCLICK=\"parent._CEmoticonRenderer_Select(this)\" CLASS=emoticon SRC=\"" + L_EMOTICONPATH_TEXT + arBigEmoticons[i] + "\" WIDTH=19 HEIGHT=19 HSPACE=4 VSPACE=4>"
if ((i+1)%6==0) sz+="<BR>"
}  				
sz+="</DIV>"
return sz
...
function _CLinkPopupRenderer_AddLink(d) 
{
	...
	if (szURL!="") 
	{
		...
		if (oSel.parentElement().tagName == "A"){
			oSel.parentElement().target = "_blank";
		}
	}
	idEditbox.focus()	
}
...
//1090311	Joe		1080166		弱掃修正Client Potential XSS--S
...
//1090311	Joe		1080166		弱掃修正Client Potential XSS--E