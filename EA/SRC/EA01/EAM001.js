...
function jf_ToolBarHandle(event)
{
	...
	switch (xObjectName)
	{
		...
		case "btClean":
			...
			break;
		case "btSearch":								
			var strUrl = "EAC001.aspx?";
			//var strKeyCol = jf_Trim(document.all["txSecNo"].value);
		    //1060308 Zen 1050087 二代公文修改			//jf_OpenChildWin(strUrl, "EAM001", 700, 500);
			jf_OpenChildWin(strUrl, "EAM001", 800, 600);
			break;
...
function jf_CheckBeforSave()
{
	...
	{
		strErrMsg += "類別代碼欄位不可空白\n";
	    //1060308 Zen 1050087 二代公文修改		//document.all["txType_No"].focus();
		$('#txType_No').focus();
		document.all["txType_No"].value="";
	}
	else 
	{
	...
	{
		strErrMsg += "類別名稱欄位不可空白\n";
	    //1060308 Zen 1050087 二代公文修改		//document.all["txType_Name"].focus();
		$('#txType_Name').focus();
		document.all["txType_Name"].value="";
	}
	else 
	{
		if ( document.all["txType_Name"].value.length > 10 )
		{
			strErrMsg += "類別名稱字元個數不可超過10\n";
		}
	}		
	if (strErrMsg != "")
	{
