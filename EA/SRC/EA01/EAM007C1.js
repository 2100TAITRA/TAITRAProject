...
{
    //儲存完畢關閉視窗較用母視窗重新搜尋
    if (document.all["SAVEOK"])
    {
        opener.window.CallBack("EAM007C1");
        jf_CloseSelf();
    }
}
...
function jf_ToolBarHandle(event)
{
	...
	switch (xObjectName)
	{
	    case "btSave":
	        ...
	        if (document.all["txAcc"].value != "") //是否通過儲存前必要檢查
	        {
...
function jf_CheckBeforSave()
{
	...
	for (var iRow=2;iRow<document.all.dg1.rows.length+1;iRow++)
	{
		...
		if (document.all["H_DATA"].value !="")
		    document.all["H_DATA"].value += "," + strClsKey + "|" + strType + "|" + strClsName + "|" + strClsNo;
		else
		    document.all["H_DATA"].value += strClsKey + "|" + strType + "|" + strClsName + "|" + strClsNo;
	}	
	//Cola -- end --
...
function CheckBeforTran()
{
    ...
    for (var iRow = 2; iRow < document.all.dg1.rows.length + 1; iRow++) {
        if (document.all["dg1__ctl" + i + "_cbSelect"].checked) {
            ...
            if (saveKey != "")
                ...
                saveKey += strClsKey;
            if (document.all["dg1__ctl" + iRow + "_txClsKey"].value == "1") {

                if (saveNewData != "")
                    saveNewData += ",序" + document.all["dg1__ctl" + i + "_lbSEQ_NO"].textContent;
                else
                    saveNewData += "序" + document.all["dg1__ctl" + i + "_lbSEQ_NO"].textContent;
            }   
        }
    ...
    if (bTran)
    { //設定資料
        var Rtn = EA01.EAT007.SaveKeyList(saveKey, jf_GetSessionID()).value;
        if (Rtn != "")
        {
            alert("開啟移交視窗異常，" + Rtn);
            bTran = false;
