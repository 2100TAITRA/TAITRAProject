...
function jf_ToolBarHandle(event) {
	...
	switch (xObjectName) {
		...
		case "btSearch":
			Page_BlockSubmit = true;
			var strUrl = "EAI014.aspx";
			jf_OpenChildWin(strUrl, "EAI014", 700, 500);
			break;
	}
}
...
function jf_CheckBeforSave() {
	...
	if (document.all["txCountryNo"].value == "") {
		strErrMsg += "國別代碼不可空白\n";
		$('#txCountryNo').focus();
	}

	if (document.all["txCountryName"].value == "") {
		strErrMsg += "國別名稱不可空白\n";
		$('#txCountryName').focus();
	}
...
function CallBack(argCallerId) {
	...
	if (argCallerId == "EAI014") {
		document.all["txCountryNo"].value = jf_Trim(document.all.lbReturnValue.options[0].value);
		if (document.all["txCountryNo"].value != "") {
			Page_BlockSubmit = false;
			jf_OpenButtonSubmit();
		}
