...
1060907     Kevin_C Zen     1050087 二代升級*/
/*****************************************************************************
*
*   Declaration 區
* 
...
function ClientButtonControl(e)
{
    ...
    switch (xObjectName)
    {
        ...
        case "btCalendarS":
            Page_BlockSubmit = true;
            jf_CallCalendar(document.all.txInpfileDateBegin, event.screenX, event.screenY);
            break;
        case "btCalendarE":
            Page_BlockSubmit = true;
            jf_CallCalendar(document.all.txInpfileDateEnd, event.screenX, event.screenY);
            break;

            //1060907 Zen 1050087 二代升級，移至ClientButtonControl            //以下屬於DataGrid ToolBar
        case "btSelectAll":
            Page_BlockSubmit = true;
...
function OnWSResult(argResult)
{
    ...
    //webserver回傳後動作
    if (argResult.id == wsCheckDocState) //增加判斷公文狀態必須為已點收待掃描(DOC_STATE='20') #2006.03.30 Andy
    {
        if (jf_IsWebServiceSuccess1(argResult))
        {
            if (jf_Trim(argResult.value.RtnField0[0]) != "20")
            {
                bDocStateOk = false;
                //1060907 Zen 1050087 二代升級                //document.all.txDocNo.focus();
                $('#txDocNo').focus();
                alert("公文狀態必須為已點收未掃描才可以進行待掃描成批作業");
            }
            else
                bDocStateOk = true;
        }
    ...
    else if (argResult.id != null)
    {
        if (jf_IsWebServiceSuccess1(argResult))
        {
            ...
            if (ret == true)
            {
                ...
                //1060907 Zen 1050087 二代升級，避免無法postback                Page_BlockSubmit = false;
...
function txDocNo_onblur()
{
    ...
    if (strDocNo != "")
    {
        ...
        {
            //增加判斷公文狀態必須為已點收待掃描(DOC_STATE='20') #2006.03.30 Andy
            var argWSParamA = new Array(5);
            argWSParamA[0] = "DOC_MAIN";
            argWSParamA[1] = new Array("SOURCE_ORGNO", "DOC_NO");
            argWSParamA[2] = new Array(strSource, strDocNo);
            argWSParamA[3] = new Array("DOC_STATE");
            argWSParamA[4] = new Array("");

            var callObjA = jf_CallWS("../EALIB/EA_LIB.asmx", "ws_GetFieldValue", false, argWSParamA);
            wsCheckDocState = callObjA.id;
            OnWSResult(callObjA);

            if (bDocStateOk == false)
                return;
            else
                //1050815 Zen 1050700 bug修正
                //bDocStateOk == false;
                bDocStateOk = false;
...
{
    var strMgrNo = jf_Trim(document.all["dlMgrNo_Text"].value);
    var strSDate = jf_Trim(document.all["txInpfileDateBegin"].value);
    var strEDate = jf_Trim(document.all["txInpfileDateEnd"].value);
    var strSTime = jf_Trim(document.all["txSTime"].value);
    var strETime = jf_Trim(document.all["txETime"].value);
    if (strMgrNo + strSDate + strEDate + strSTime + strETime == "")
...
{
    var strSDate = document.all["txInpfileDateBegin"].value;
    var strEDate = document.all["txInpfileDateEnd"].value;
    if (strSDate == "" || strEDate == "")
    {
        //readonly
        document.all["txSTime"].className = "DisplayOnly";
        document.all["txSTime"].readOnly = true;
        document.all["txSTime"].style.color = "Navy";
        document.all["txSTime"].tabIndex = -1;
        document.all["txETime"].className = "DisplayOnly";
        document.all["txETime"].readOnly = true;
        document.all["txETime"].style.color = "Navy";
        document.all["txETime"].tabIndex = -1;
    }
    else
    {
        //un readonly
        document.all["txSTime"].className = "InputFieldText";
        document.all["txSTime"].readOnly = false;
        document.all["txSTime"].tabIndex = 0;
        document.all["txETime"].className = "InputFieldText";
        document.all["txETime"].readOnly = false;
        document.all["txETime"].tabIndex = 0;
    }
}

//檢查時間(HH)
function jf_CheckTime(argObj, errMsg)
{
    if (argObj.value == "")
        return true;
    if (argObj.value.length < 2)
        argObj.value = jf_PADL(argObj.value, 2, '0');
    var strHH = argObj.value.substring(0, 2);
    if (Number(strHH) >= 24)
    {
        //1060907 Zen 1050087 二代升級        //argObj.focus();
        $('#' + argObj.id).focus();
...
//日期onblur
function CallCheckCDATE(argObj, errMsg)
{
    if (argObj.value != "")
    {
    ...
    CheckDateEmpty();
    //避免由編目日期(迄)清空後按下tab 在尚未設定readonly前,focus已經在編目時間(起)內,導致編目時間(起)仍可變更
    if (argObj.id == "txInpfileDateEnd")
    {
        if (argObj.value == "")
        {
            if (document.activeElement.id == "txSTime" || document.activeElement.id == "txETime")
                //1060907 Zen 1050087 二代升級                //document.all["txDocNo"].focus();
                $('#txDocNo').focus();
        }
    }
