...
1110330		Cloud		Cloud	1111531			新增
*/
/*****************************************************************************
*
*   Declaration 區
...
function jf_CheckBeforSave() {
	...
	if ($$("txDateS").value == "" && $$("txDateE").value == "") {
		strErrMsg += "年度區間不可均為空白\n";
		bRtnbool = false;
		$('#txDateS').focus();
	}
	else {
		if ($$("txDateS").value != "") {
			$$("txDateS").value = jf_PADL($$("txDateS").value, 3, '0')
		}

		if ($$("txDateE").value != "") {
			$$("txDateE").value = jf_PADL($$("txDateE").value, 3, '0')
		}
		...
		if (document.all["OrgNickName"].value != "EXAM") {
			if (parseInt(document.all["txDateE"].value) - parseInt(document.all["txDateS"].value) > 10) {
				alert('年度起迄不可大於10年。');
				bRtnbool = false;
				$('#txDateS').focus();
			}
