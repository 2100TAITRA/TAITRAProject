...
function ReturnValue(argLink)
{
    try
    {
        ...
        opener.window.CallBack("EDR243_MOCS");
        close();
    }
    catch (e) { }
...
{
    var strSDate = jf_Trim(document.all.txCloseDateS.value);
    var strEDate = jf_Trim(document.all.txCloseDateE.value);
    ...
    var bRtnbool = true;
    var strErrMsg = "";
    if (strSDocNo == "" && strEDocNo == "" && strSDate == "" && strEDate == "")
    {
        bRtnbool = false;
        alert("請至少輸入一個條件");
        return bRtnbool;
    }
    if (!CheckDATE("txCloseDateS", "結案日期(起)"))
    {
        bRtnbool = false;
        return bRtnbool;
    }

    if (!CheckDATE("txCloseDateE", "結案日期(迄)"))
    {
        bRtnbool = false;
        return bRtnbool;
    }
    ...
    {
        document.all.txDocNoE.value = strSDate;
        document.all.txDocNoS.value = strEDate;
    }
    if (strSDate == "" && strEDate != "")
    {
        document.all.txCloseDateS.value = strEDate;

    }
    if (strSDate != "" && strEDate == "")
    {
        document.all.txCloseDateE.value = strSDate;
