...
1110316	David	David	1101388			新增本作業
1110812	David	David	-------			儲存時將是否有異動資訊設定給theAOL.docObj.wwkfModified，供公文關閉時判斷是否需提醒
1110922	David	David	1110669			新增個人化預排流程功能
1110926	David	Joe		1110629			調整UI
1111101	David	Joe		--				修正本單位無法取得自己的一級單位，調整代碼定義
...
var theUserInfo;//紀錄theUserInfo物件
...
function ClientOnLoad()
{
    ...
    if ($("#OrgNickName").val() == "EXAM")
        $("#CoworkThread").hide();

    init();

    setPersonalFlowSet();//1110922 David 1110669 設定預排流程功能
...
function ClientButtonControl(e)
{
    ...
    switch (xObjectName)
    {
        ...
        case "btDgMoveDown":
            ...
            fnMoveDown();
            break;
        case "btChangePersonalFlowSet"://1110922 David 1110669 新增預排流程替換功能
            Page_BlockSubmit = true;
...
function OnWSResult(argResult)
{
    ...
    else if (argResult.id == iCallID_ProcPeraonalWWKF)
    {
        ...
        if (objPersonFlowSet.m_bSuccess)
        {
            ...
            //1131015 Zen 1130941 弱掃XSS修正，調整取得預排流程方式
            //var xdoc = jQuery.parseXML(objPersonFlowSet.m_strRetStr);
            //var $xml = $(xdoc);
            //var $title = $xml.find( "item" );
            //if($title.length == 0)
            if (objPersonFlowSet.m_flowItem.length == 0)
            {
            ...
            else
            {
                ...
                for (var iRow = 0; iRow < objPersonFlowSet.m_flowItem.length; iRow++)
                {
                    NewWWKF[iRow] = Object();
                    //1120915  Joe		1120709			弱掃修正XSS
                    // NewWWKF[iRow].ADDBY = $($title[iRow]).find("ADDBY")[0].textContent;
                    // NewWWKF[iRow].CREATE_BY = 
                    // NewWWKF[iRow].OWN_OU_ID = $($title[iRow]).find("OWN_OU_ID")[0].textContent;
                    // NewWWKF[iRow].OWN_OU_NAME = $($title[iRow]).find("OWN_OU_NAME")[0].textContent;
                    // NewWWKF[iRow].OWN_ROLE_ID = $($title[iRow]).find("OWN_ROLE_ID")[0].textContent;
                    // NewWWKF[iRow].OWN_ROLE_NAME = $($title[iRow]).find("OWN_ROLE_NAME")[0].textContent;
                    // NewWWKF[iRow].OWN_USER_ID = $($title[iRow]).find("OWN_USER_ID")[0].textContent;
                    // NewWWKF[iRow].OWN_USER_NAME = $($title[iRow]).find("OWN_USER_NAME")[0].textContent;
                    // NewWWKF[iRow].RADIO_SELECTED_1 = $($title[iRow]).find("RADIO_SELECTED_1")[0].textContent;
                    // NewWWKF[iRow].SEND_BY = $($title[iRow]).find("SEND_BY")[0].textContent;
                    // NewWWKF[iRow].SEND_TIME = $($title[iRow]).find("SEND_TIME")[0].textContent;
                    // NewWWKF[iRow].SIGN_F = "N"
                    // NewWWKF[iRow].TX_NAME = $($title[iRow]).find("TX_NAME")[0].textContent;
                    //1131015 Zen 1130941 弱掃XSS修正
                    //NewWWKF[iRow].ADDBY = htmlencode($($title[iRow]).find("ADDBY")[0].textContent);
                    //NewWWKF[iRow].CREATE_BY = htmlencode($($title[iRow]).find("ADDBY")[0].textContent);
                    //NewWWKF[iRow].OWN_OU_ID = htmlencode($($title[iRow]).find("OWN_OU_ID")[0].textContent);
                    //NewWWKF[iRow].OWN_OU_NAME = htmlencode($($title[iRow]).find("OWN_OU_NAME")[0].textContent);
                    //NewWWKF[iRow].OWN_ROLE_ID = htmlencode($($title[iRow]).find("OWN_ROLE_ID")[0].textContent);
                    //NewWWKF[iRow].OWN_ROLE_NAME = htmlencode($($title[iRow]).find("OWN_ROLE_NAME")[0].textContent);
                    //NewWWKF[iRow].OWN_USER_ID = htmlencode($($title[iRow]).find("OWN_USER_ID")[0].textContent);
                    //NewWWKF[iRow].OWN_USER_NAME = htmlencode($($title[iRow]).find("OWN_USER_NAME")[0].textContent);
                    //NewWWKF[iRow].RADIO_SELECTED_1 = htmlencode($($title[iRow]).find("RADIO_SELECTED_1")[0].textContent);
                    //NewWWKF[iRow].SEND_BY = htmlencode($($title[iRow]).find("SEND_BY")[0].textContent);
                    //NewWWKF[iRow].SEND_TIME = htmlencode($($title[iRow]).find("SEND_TIME")[0].textContent);
                    //NewWWKF[iRow].SIGN_F = "N"
                    //NewWWKF[iRow].TX_NAME = htmlencode($($title[iRow]).find("TX_NAME")[0].textContent);
                    NewWWKF[iRow].OWN_USER_ID = objPersonFlowSet.m_flowItem[iRow].strOWN_USER_ID;
                    NewWWKF[iRow].OWN_USER_NAME = objPersonFlowSet.m_flowItem[iRow].strOWN_USER_NAME;
                    NewWWKF[iRow].OWN_OU_ID = objPersonFlowSet.m_flowItem[iRow].strOWN_OU_ID;
                    NewWWKF[iRow].OWN_OU_NAME = objPersonFlowSet.m_flowItem[iRow].strOWN_OU_NAME;
                    NewWWKF[iRow].OWN_ROLE_ID = objPersonFlowSet.m_flowItem[iRow].strOWN_ROLE_ID;
                    NewWWKF[iRow].OWN_ROLE_NAME = objPersonFlowSet.m_flowItem[iRow].strOWN_ROLE_NAME;
                    NewWWKF[iRow].CREATE_BY = objPersonFlowSet.m_flowItem[iRow].strCREATE_BY;
                    NewWWKF[iRow].SEND_BY = objPersonFlowSet.m_flowItem[iRow].strSEND_BY;
                    NewWWKF[iRow].SEND_TIME = objPersonFlowSet.m_flowItem[iRow].strSEND_TIME;
                    NewWWKF[iRow].ADDBY = objPersonFlowSet.m_flowItem[iRow].strADDBY;
                    NewWWKF[iRow].RADIO_SELECTED_1 = objPersonFlowSet.m_flowItem[iRow].strRADIO_SELECTED_1;
                    NewWWKF[iRow].SIGN_F = objPersonFlowSet.m_flowItem[iRow].strSIGN_F;
                    NewWWKF[iRow].TX_NAME = objPersonFlowSet.m_flowItem[iRow].strTX_NAME;
                }

                LoadXmlToTable(NewWWKF);
            }
        }
    }
}
...
function LoadXmlToTable(objwwkf)
{
    ...
    for (var iRow = 0; iRow < objwwkf.length; iRow++)
    {
        ...
        if (objwwkf[iRow].TX_NAME != "分會")
        {
            ...
            strInnerHtml += "<INPUT id=\"txOwnRoleId" + nTableRowMax + "\" type=\"hidden\" name=\"twOwnRoleId" + nTableRowMax + "\" value=\"" + HtmlEncode(pNodeVal) + "\">";
            pNodeVal = objwwkf[iRow].OWN_ROLE_NAME;
			//1140911	Joe		--		自測修正弱掃改壞的問題
            // strInnerHtml += "<INPUT id=\"txOwnRoleName" + nTableRowMax + "\" type=\"hidden\" name=\"twOwnRoleName" + nTableRowMax + "\" value=\"" + pNodeVal + "\">";
            strInnerHtml += "<INPUT id=\"txOwnRoleName" + nTableRowMax + "\" type=\"hidden\" name=\"twOwnRoleName" + nTableRowMax + "\" value=\"" + HtmlEncode(pNodeVal) + "\">";
            ...
            strInnerHtml += "<INPUT id=\"txSendBy" + nTableRowMax + "\" type=\"hidden\" name=\"twOwnUserId" + nTableRowMax + "\" value=\"" + HtmlEncode(pNodeVal) + "\">";
            ...
            strInnerHtml += "<INPUT id=\"txSendTime" + nTableRowMax + "\" type=\"hidden\" name=\"twOwnUserId" + nTableRowMax + "\" value=\"" + HtmlEncode(pNodeVal) + "\">";

            pNodeVal = objwwkf[iRow].ADDBY; //ADDBY  -- 加入方式 (0:加入流程, 1:加入流程點)
            ...
            pNodeVal = objwwkf[iRow].RADIO_SELECTED_1;
            ...
            if (CheckBoxHidden != "")
				...
				//1140911	Joe		--		自測修正弱掃改壞的問題
                // document.all.dg1.rows[nRowIdx].cells[0].innerHTML = "<INPUT id=\"cbCheck" + nTableRowMax + "\" type=\"checkbox\" " + ItemDisable + " " + CheckBoxHidden + ">" + HtmlEncode(strInnerHtml);
                document.all.dg1.rows[nRowIdx].cells[0].innerHTML = "<INPUT id=\"cbCheck" + nTableRowMax + "\" type=\"checkbox\" " + ItemDisable + " " + CheckBoxHidden + ">" + strInnerHtml;
            else
                //1140717   Joeko	1141011    弱掃修正Client Potential XSS
				...
				//1140911	Joe		--		自測修正弱掃改壞的問題
                // document.all.dg1.rows[nRowIdx].cells[0].innerHTML = "<INPUT id=\"cbCheck" + nTableRowMax + "\" type=\"checkbox\" " + ItemDisable + " onclick=\"fnSetColor(" + nTableRowMax + ")\" >" + HtmlEncode(strInnerHtml);
                document.all.dg1.rows[nRowIdx].cells[0].innerHTML = "<INPUT id=\"cbCheck" + nTableRowMax + "\" type=\"checkbox\" " + ItemDisable + " onclick=\"fnSetColor(" + nTableRowMax + ")\" >" + strInnerHtml;
            //單位
            //1140717   Joeko	1141011    弱掃修正Client Potential XSS
            /*document.all.dg1.rows[nRowIdx].cells[1].innerHTML = objwwkf[iRow].OWN_OU_NAME;*/
            document.all.dg1.rows[nRowIdx].cells[1].innerHTML = HtmlEncode(objwwkf[iRow].OWN_OU_NAME);
            //異動別
            //1140717   Joeko	1141011    弱掃修正Client Potential XSS
            /*document.all.dg1.rows[nRowIdx].cells[2].innerHTML = objwwkf[iRow].TX_NAME;*/
            document.all.dg1.rows[nRowIdx].cells[2].innerHTML = HtmlEncode(objwwkf[iRow].TX_NAME);
            //分會明細按鈕
            //1110926	Joe		1110629		調整紙本預排流程UI
        ...
        else
        {
            ...
            //分會明細按鈕
            //1110926	Joe		1110629		調整紙本預排流程UI
            //document.all.dg1.rows[nRowIdx].cells[3].innerHTML = "<INPUT id=\"btShowDetail" + nTableRowMax + "\" type=\"button\" " + ItemDisable + " onclick=\"fnOpenThread(" + nTableRowMax + ")\" name=\"btShowDetail" + nTableRowMax + "\" value=\"明細\">";
            //1121207	Joe		序295	調整未走過的流程才可異動排序
            if (objwwkf[iRow].SIGN_F.toUpperCase() != "Y")
            {
                document.all.dg1.rows[nRowIdx].cells[3].innerHTML = "<INPUT id=\"btMoveUp" + nTableRowMax + "\" type=\"button\" onclick=\"fnSingleMoveUp(" + nTableRowMax + ")\" name=\"btMoveUp" + nTableRowMax + "\" value=\"↑\">";
...
function fnAddPoint(pOuId, pOuName)
{
    ...
    */
    ...
    var pTxName = GetTxName(pOuId, gDocInfo.IcOuId);
    if (pTxName == "")
    {
        alert('無法判定異動別，請重新點選');
        return false;
    ...
    //1120926	Joe		1120526		調整該依原始代碼判斷
    // AddFlowPoint(pOuId, pOuName, pRadioSelected1, pTxName, iTargetOuId);
...
function AddFlowPoint(pOuId, pOuName, pRadioSelected1, pTxName, iTargetOuId)
{
    ...
    //1120915  Joe		1120709			弱掃修正XSS
    // strInnerHtml += "<INPUT id=\"txOwnOuId" + nTableRowMax + "\" type=\"hidden\" name=\"txOwnOuId" + nTableRowMax + "\" value=\"" + pNodeVal + "\">";
    // strInnerHtml += "<INPUT id=\"txOwnRoleId" + nTableRowMax + "\" type=\"hidden\" name=\"twOwnRoleId" + nTableRowMax + "\" value=\"" + gProgressRoleId + "\">";
    // strInnerHtml += "<INPUT id=\"txOwnRoleName" + nTableRowMax + "\" type=\"hidden\" name=\"twOwnRoleName" + nTableRowMax + "\" value=\"" + gProgressRoleName + "\">";
    strInnerHtml += "<INPUT id=\"txOwnOuId" + nTableRowMax + "\" type=\"hidden\" name=\"txOwnOuId" + nTableRowMax + "\" value=\"" + htmlencode(pNodeVal) + "\">";
    ...
    //單位
    //1120915  Joe		1120709			弱掃修正XSS
    // document.all.dg1.rows[InsertIndex].cells[1].innerHTML = pOuName;
    document.all.dg1.rows[InsertIndex].cells[1].innerHTML = htmlencode(pOuName);
    //異動別
    //1120915  Joe		1120709			弱掃修正XSS
    // document.all.dg1.rows[InsertIndex].cells[2].innerHTML = pTxName;
...
function GetTxName(argToOuId, argInChargeOu)
{
    ...
    // var T = parseInt(argToOuId.substr(0, 2), 10);
    ...
    else if (T.indexOf("9") == 0 && T != "90")//一層決行
    {
        ...
        // return "送請簽核";
        return GetTxNameFromWWKFMain("OD17", argToOuId);
    }
    else
    {
        ...
        if (FlowObj.bSuccess)
        {
            if (FlowObj.OuId.length > 0)
            {
                ...
                /*for (var iFlows = 0; iFlows < FlowObj.OuId.length; iFlows++)
                {
                    ...
                    // AddFlowPoint(strOuId, strOuName, pRadioSelected1, strTxName, iTargetOuId);
                    AddFlowPoint(strOuId, strOuName, pRadioSelected1, strTxName, TargetOuId);
                }*/
...
function fnAddThread(argOuList)
{
    ...
    */
    if (argOuList != "")
    {
        ...
        var pNodeVal = parent.theUserInfo.UserName;
        strInnerHtml += "<INPUT id=\"txCreateBy" + nTableRowMax + "\" type=\"hidden\" name=\"twOwnUserId" + nTableRowMax + "\" value=\"" + pNodeVal + "\">";
        strInnerHtml += "<INPUT id=\"txSendBy" + nTableRowMax + "\" type=\"hidden\" name=\"twOwnUserId" + nTableRowMax + "\" value=\"\">";
        strInnerHtml += "<INPUT id=\"txSendTime" + nTableRowMax + "\" type=\"hidden\" name=\"twOwnUserId" + nTableRowMax + "\" value=\"\">";
        ...
        //分會明細按鈕
        //1110926	Joe		1110629		調整紙本預排流程UI
        //document.all.dg1.rows[InsertIndex].cells[3].innerHTML = "<INPUT id=\"btShowDetail" + nTableRowMax + "\" type=\"button\" onclick=\"fnOpenThread(" + nTableRowMax + ")\" name=\"btShowDetail" + nTableRowMax + "\" value=\"明細\">";
        document.all.dg1.rows[InsertIndex].cells[3].innerHTML = "<INPUT id=\"btMoveUp" + nTableRowMax + "\" type=\"button\" onclick=\"fnSingleMoveUp(" + nTableRowMax + ")\" name=\"btMoveUp" + nTableRowMax + "\" value=\"↑\">";
        document.all.dg1.rows[InsertIndex].cells[3].innerHTML += "<INPUT id=\"btMoveDown" + nTableRowMax + "\" type=\"button\" onclick=\"fnSingleMoveDown(" + nTableRowMax + ")\" name=\"btMoveDown" + nTableRowMax + "\" value=\"↓\">";
        document.all.dg1.rows[InsertIndex].cells[3].innerHTML += "<INPUT id=\"btDelete" + nTableRowMax + "\" type=\"button\" onclick=\"fnSingleDelete(" + nTableRowMax + ")\" name=\"btDelete" + nTableRowMax + "\" value=\"刪\">";
...
//1110922 David 1110669 設定個人化預排流程選項選單
...
function setPersonalFlowSet()
{
    if (bFirstWWKF)
    {
        ...
        if (document.all.ODMSSP && document.all.ODMSSP.value != "")
        {
            ...
            param[3] = gDocInfo.IcUserId;
            var callObj = jf_CallWS(document.all.ODMSSP.value, "GetPersonalPWWKF", false, param);
...
//1110922 David 1110669 設定可替換個人化預排流程功能
...
function ChangePersonalFlowSet()
{
    ...
    param[2] = gDocInfo.OwnOuId;
    param[3] = gDocInfo.IcUserId;
    param[4] = strFlowSetSeq;
    var callObj = jf_CallWS(document.all.ODMSSP.value, "ProcPeraonalPWWKF", false, param);
...
function initDeptDg()
{
    ...
    var DeptObj = ED2.EDT218.GetDeptRoleInfo(document.all.OrgNo.value, gDocInfo.OwnOuId.substr(0, 2), DeptType).value;
    if (DeptObj)
    {
        if (DeptObj.bSuccess)
        {
...
function AddDept(argOuId, argOuName, argType)
{
    ...
    // strInnerHtml += "<INPUT id=\"txOuName" + nDeptMaxRow + "\" type=\"hidden\" name=\"txOuName" + nDeptMaxRow + "\" value=\"" + argOuName + "\">";
...
function fnSpread(argRowId)
{
    ...
    //1130820	Joe		序185		調整代碼定義，增加會辦單位展開--S
    var SpreadType = "S";
    if (document.all.rbDeptCowork.checked)
        SpreadType = "SC";
    //1130820	Joe		序185		調整代碼定義，增加會辦單位展開--E
    //1111101	Joe		調整代碼定義
    // var SpreadList = ED2.EDT218.GetDeptRoleInfo(document.all.OrgNo.value, document.all["txOuId" + argRowId].value, "N").value;
    //1130820	Joe		序185		調整代碼定義，增加會辦單位展開
    // var SpreadList = ED2.EDT218.GetDeptRoleInfo(document.all.OrgNo.value, document.all["txOuId" + argRowId].value, "S").value;
    var SpreadList = ED2.EDT218.GetDeptRoleInfo(document.all.OrgNo.value, document.all["txOuId" + argRowId].value, SpreadType).value;
    if (SpreadList)
    {
