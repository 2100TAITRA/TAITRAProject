...
1080311 David		Joe			1071272 新增程式
...
function ClientOnLoad()
{
	...
	if (document.all.dlProperty.selectedIndex != 0) {
		dlProperty_onchange();
		var dlBTypeNo = document.all.dlBTypeNo;
		var workTypeIndex = encodeURI(document.all.h_workTypeIndex.value)
		if (workTypeIndex != '') {
			dlBTypeNo.selectedIndex = workTypeIndex;
			dlBTypeNo_onchange();
		}
	}
}
...
function adjust()
{
	var strS = jf_Trim(document.all.txIssueDateS.value);
	var strE = jf_Trim(document.all.txIssueDateE.value);
	if(strS != "" && strE == "")
		document.all.txIssueDateE.value = strS;
	else if(strS == "" && strE != "")
		document.all.txIssueDateS.value = strE;
	else if(strS > strE)
	{
		document.all.txIssueDateS.value = strE;
		document.all.txIssueDateE.value = strS;
...
{	
	if (jf_Trim(document.all["txRcvS"].value) == "" && jf_Trim(document.all["txRcvE"].value) == "" && jf_Trim(document.all["txDocS"].value) == ""
		&& jf_Trim(document.all["txDocE"].value) == "")
	{
		alert('公文文號、收創日期不可皆為空白');
		return false;
	}
	else
		return true;
...
function dlProperty_onchange() {
	...
	document.all.dlBTypeNo.innerHTML = "";
	...
	param[1] = encodeURI(document.all.dlProperty.options[document.all.dlProperty.selectedIndex].value);
...
function dlBTypeNo_onchange() {
	var dlBTypeNo = document.all.dlBTypeNo;
	document.all.h_workTypeIndex.value = dlBTypeNo.selectedIndex;
	document.all.h_workTypeValue.value = dlBTypeNo.options[dlBTypeNo.selectedIndex].value;
	//1081231	Kevin_C	1081095	修正取下拉選單選項的屬性在IE不支援的問題
	//document.all.h_workTypeText.value = dlBTypeNo.selectedOptions[0].text;
	document.all.h_workTypeText.value = dlBTypeNo.options[dlBTypeNo.selectedIndex].text;
}
