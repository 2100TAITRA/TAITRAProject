...
1030729	David	Eric	1030516		新增EDR383程式
...
//指定DataGrid欄位
var strTableFields = new Array("_lbDocNo","_txIssueOrgName","_txPostCode","_txAddress");
...
function jf_ToolBarHandle(event)
{
	...
	switch (xObjectName)
	{
		case "btCancel":
			...
			break;
		case "btPrint":
			if(document.all["H_CheckData"].value=="1")
			{
...
function jf_CheckBeforPrint()
{
	...
	for(var i = 2; i <= document.all.dg1.rows.length; i++)
	{
		if(document.all["dg1__ctl" + i + "_txIssueOrgName"].value=="")
		{
			Changed = false;
			break;	
		}
	...
	if(isNaN(document.all.txPositionNo.value) || parseInt(document.all.txPositionNo.value) == 0)
	{
	    alert("位置請輸入數字或不可為0");
	    //1070111 Justin [1050087] 二代公文修改
