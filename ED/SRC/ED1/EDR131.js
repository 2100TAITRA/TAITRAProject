 ...
 * 日期			修改人	單號        概要
 * -------------------------------------------------------------------------------------------------
 * 96.03.		whay	960076      仿照EDR130，新增程式
 * 96.03.23		shelly	960076	    接whay未完成之部份
...
function ClientButtonControl(e)
{
	...
	switch (xObjectName)
	{
		case "btHelp":
			...
			var strUrl = "";
			strUrl = "http://"+WebServerName+"/ODDEP/WEM010C1.aspx?OrgID="+document.all.h_OrgNo.value+"&K1=Dlg_Dept&Search="+CurrOrgIdObj.value;
...
function GetDG()
{
	document.all["txMake"].value ="";
	for (var i=2 ; i<document.all.dg1.rows.length+1; i++)
	{
		if (document.all["dg1__ctl"+i+"_cbSelect"].checked)
			if(document.all["txMake"].value != "")
				document.all["txMake"].value = document.all["txMake"].value + ";" + i  ;
			else
				document.all["txMake"].value = i;
	}
}
