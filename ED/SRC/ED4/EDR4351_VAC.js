...
1140507  Joe	 Joe        1140077   	新增程式
1140519	 Joe	 Joe	 	1140077		(自測)年度統計表不可跨年度查詢
*/
/*****************************************************************************
*
*   Declaration 區
...
function jf_CheckPreview() {
    ...
    var strWorkDayS = document.all['txRcvDateS'].value;
    var strWorkDayE = document.all['txRcvDateE'].value;

    if (strWorkDayS == '' && strWorkDayE != '')
        document.all['txRcvDateS'].value = strWorkDayE
    else if (strWorkDayS != '' && strWorkDayE == '')
        document.all['txRcvDateE'].value = strWorkDayS
    else if (Number(strWorkDayS) > Number(strWorkDayE)) {
        document.all['txRcvDateS'].value = strWorkDayE
        document.all['txRcvDateE'].value = strWorkDayS
    }
	
	//1140519	Joe		1140077		(自測)年度統計表不可跨年度查詢
	if($('#txRcvDateS').val().substring(0, 3) != $('#txRcvDateE').val().substring(0, 3))
	{
		alert('年度統計表不可跨年度查詢。');
		ReturnValue = false;
	}
