 ...
 * 1040518	Kevin	Eric	1040300	新增未採行線上簽核統計表及清單列印作業
 * 1061013	Kevin	Joe		1050087	二代升級
 * -------------------------------------------------------------------------------------------------
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
/*****************************************************************************
*
...
* 
*****************************************************************************/
//1061013	joe		1050087		二代修改配合行動平台
...
* 
*****************************************************************************/
//1061013 Joe 1050087 二代公文修改，傳入參數event
...
function odjf_ComboBoxCheck(argComboBoxID, argKeyMsg)
{
	...
	if (!bRtnBool)
	{
		...
		//1061013	Joe	1050087	二代系統升級，調整focus寫法
...
function ConfirmPrint()
{
	...
	if(jf_Trim(document.all.txMonthS.value)==""&&jf_Trim(document.all.txMonthE.value)=="")
	{
		...
		$('#' + document.all.txMonthS.id).focus();
		return false;
	}
	
	for(var i = 2; i <= document.all.dg1.rows.length; i++)
	{
		var strCheckReason	=	"dg1__ctl" + i + "_cbSelect";
		if(document.all[strCheckReason].checked)
		{
			bChkReason = true;
		}
	}
	if(!bChkReason)
	{
		jf_ShowMeg("請至少勾選一項非電子發文原因","");
		return false;
	}
	if(!jf_CheckDate("txMonthS"))
		return false;
	if(!jf_CheckDate("txMonthE"))
		return false;
