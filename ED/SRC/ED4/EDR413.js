...
1051005		Kevin	Joe		1050815		新增本作業EDR413 結案公文嚴重逾期回報查詢作業
1051019     Leslie  Kenny   1050087     二代公文修改
1060522		Kevin	Joe		1050815		修正一律隱藏二級單位選項
*/
/*****************************************************************************
*
*   Declaration 區
...
function CheckDateSE()
{
	...
	if (document.all["txDocNoS"].value != "" || document.all["txDocNoE"].value != "")
	{
		...
		{
			Temp = document.all["txDocNoS"].value;
			document.all["txDocNoS"].value = document.all["txDocNoE"].value;
...
{
	CheckDate(document.all["txDateS"], "結案日期(起)");
}

function txDateE_blur()
{
	CheckDate(document.all["txDateE"], "結案日期(迄)");
}
...
//1060726	JOE		1060582			新增填寫鍵--S
function DocWritePaper(argDocNo,argUserId,argOrgNo)
{
	var strUrl = "";
		
	strUrl += "../../../ODDEP/ODT410.aspx?rtnObj=lbReturnValue&TYPE=2";
