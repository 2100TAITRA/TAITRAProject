...
1140515 Joe     Daniel  1140129     新增程式
*/
/*****************************************************************************
*
*   Declaration 區
...
function dlDocTypeChange() {
    $('#dlDocType').on('change',function () {
        const ptyVal = $(this).val();
        const proVal = $('#dlProjectType').val();
        if ((ptyVal === "3" && proVal === "Normal") || (ptyVal !== "3" && proVal === "Project")) {
            $('#dlProjectType').val("");
        }
    });
}

function dlProjectTypeChange() {
    $('#dlProjectType').on('change',function () {
        const proVal = $(this).val();
        const ptyVal = $('#dlDocType').val();
        if ((proVal === "Project" && ptyVal !== "3") || (proVal === "Normal" && ptyVal === "3")) {
            $('#dlDocType').val('');
        }
    });
}
