...
function jf_CheckPreview() {
	...
	if (!ED4.EDR4303_VAC.CheckSearchDays($('#txRcvDateS').val(), $('#txRcvDateE').val()).value)
	{
		alert('查詢範圍最大為90天。');
		return false;
	}
...
function fnRPTTypechg()
{
	if ($('#rbRptDetail').prop('checked') == true)
		$('#divDetail').show();
	else
		$('#divDetail').hide();
}

function fndlClosechg(ctrl)
{
	if (ctrl == 'Type' && $('#dlCloseType').val() != "")
		$('#dlCloseStatus').val('Close');
	else if (ctrl == 'Status' && $('#dlCloseStatus').val() == "Wait")
		$('#dlCloseType').val('');
}