...
1120721 Cloud   Cloud   1120285     [鐵道局]修改有申請中資料時，第一項起日預設帶入收創文日
...
function CheckBeforSave()
{
    ...
    if (document.all["OD_ODT258_PLAN_SCHEDULE_DETAIL"].value == "Y")
    {
        ...
        */
        if (!CheckNotEmptyAndAlert("txCaseName", "案由"))
            return false;
        ...
        else
        {
            ...
            //1120721 Cloud    1120285     修改增加檢核最後一項事項迄日需與申請後日期相同
            var LastRow = "";
            for (var iRow = 2; iRow <= document.all["dgSchedule"].rows.length; iRow++)
            {
                strEmptyText = "";
                if (document.all["dgSchedule__ctl" + iRow + "_txPlan"].value != "" && document.all["dgSchedule__ctl" + iRow + "_txDateS"].value != "" && document.all["dgSchedule__ctl" + iRow + "_txDateE"].value != "")
                {
                    ...
                    //1120724 Cloud     1120285     增加檢核最後一項事件迄日須為申請後限辦日期
...
var arrDesc = new Array("lbDesc2", "DescList1", "DescList2", "DescList3", "DescList4", "DescList5");
