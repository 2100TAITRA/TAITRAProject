...
1110920	Cloud	Cloud	1110857 新增銓敘部客製化送發清單
1130409	Cloud	Cloud	1130089	新增查詢模式
*/
...
function jf_ToolBarHandle(event)
{
	...
	switch (xObjectName)
	{
		...
		case "btSearch":
			Page_BlockSubmit = true;
			//1130409	Cloud	1130089	新增查詢模式
		    /*var strMode = "0";
		    if (document.all["rbReprotSet_1"].checked)
		        strMode = "1";
		    var strUrl = "ODR126C1.aspx?rtnObj=lbReturnValue&argRptMODE=" + strMode;
		        jf_OpenChildWin(strUrl, "ODR126C1", 1024, 768);*/
...
function ClientOnLoad()
{
	...
	//1130409 Cloud	1130089	新增查詢模式
	//1130409 Cloud 1130089 開啟後隱藏查詢條件
...
function jf_RefreshDg()
{
    window.clearInterval(intervalID);
    if (document.all.dg1 && document.all.dg1.rows.length >= 2)
    {
        if (window.confirm("當前已有資料，切換報表種類後將清空畫面，是否確定切換?")) {
            Page_BlockSubmit = false;
...
function jf_SetSearchMode() {
	...
	if (document.all["rbBatchMode_1"].checked) {
		...
		if (document.all.dg1 && document.all.dg1.rows.length >= 2 && document.all["H_txWorkMode"].value != "1") {
			...
			document.all["DocMode"].className = "hide";
			document.all["btSearch"].disabled = false;
		if (document.all.dg1 && document.all.dg1.rows.length >= 2 && document.all["rbBatchMode_0"].checked && document.all["H_txWorkMode"].value != "0") {
			if (window.confirm("當前已有資料，切換成批模式後後將清空畫面，是否確定切換?")) {
				Page_BlockSubmit = false;
				document.all["H_txWorkMode"].value = "0";
				jf_ToolBarSubmit("btCancel");
