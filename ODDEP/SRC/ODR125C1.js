	...
	switch (xObjectName)
	{
	    ...
	    case "btConfirm":
	        ...
	        for (i = 2; i < document.all.dg1.rows.length + 1; i++) {
	            if (document.all["dg1__ctl" + i + "_cb1"].checked) {
	                if (strDocNoList != "")
	                    strDocNoList += ",";
	                //回傳母視窗時將*去除
	                strDocNoList += document.all["dg1__ctl" + i + "_hlDocNo"].innerText.replace('*', '');
	                count++;
	            }
	        }
	        if (count == 0) {
	        ...
	        if (count > 0) {
	            opener.window.CallBack("ODR125C1");
	            close();
	        }
	        break;
	}
...
function ReturnValue() {
    ...
    for (var i = 0; i < document.all.dg1.length; i++) {
        if (document.all.dg1.options[i].value == document.activeElement.innerText) {
            //回傳母視窗時將*去除
            opener.document.all.lbReturnValue.options[0].text = document.all.dg1.options[i].text.replace('*', '');
            break;
        }
    }
    //回傳母視窗時將*去除
