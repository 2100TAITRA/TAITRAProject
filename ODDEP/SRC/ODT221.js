...
1050407 Leslie  Cloud   1050087     升級二代
1050818	Cloud	Kevin_C	1050087		二代修改，改用二代瀏覽模組
1050824	Cloud	Cloud	1050087		調整開啟edi200路徑
1051031	Cloud	Cloud	1050087		修正傳送無作用問題
1051201 Cloud   Cloud   1051181		[鐵改局]因公文性質不同調整畫面說明斷段落以及擬定作業事項
1060613 Kevin	Justin	1060456		弱掃Client Potential Code Injection修正
1070731	Kevin   Joe     1070678 	修正弱掃Hardcoded Absolute Path
1070912 Kevin	Joe		1070172		新增功能線上瀏覽、撤回、參考附件、預排流程，紙本公文不提供線上申請，展期最大天數檢核
1130521	Cloud	Jason	1130387		修改說明欄位顯示資訊
1130918	Cloud	Cloud	1130941		XSS弱掃修改
1140811	Cloud	Cloud	1130941 	XSS弱掃修改-因應欄位因參數設定是否啟用，增加判斷欄位存在才做DECODE
*/
...
function jf_ToolBarHandle(event)
{
	...
	switch (xObjectName)
	{
		case "btBack":
			...
			break;
		case "btCommit":
			//GetSelectedUser();	//核可不需取得下拉式選單值
			Page_BlockSubmit=false;
			//0991028	Howard	0990647	核可時判斷是否需自動加註核可原因
			CheckAuditMsg();
		    //1050407 Cloud   1050087     升級二代
		    //jf_ToolBarSubmit();
			jf_ToolBarSubmit(xObjectName);
		...
		case "btSave":
		 ...
		 case "btSearchFlow":
			...
			if(ConfirmOpen())
			{
				...
				//1050824	Cloud	Cloud	1050087		調整開啟edi200路徑
				//var strUrl ="../../EDN/ED2/EDI200.aspx?SOURCE_ORGNO="+strSourceOrgno+"&argMsgFrom=ODT220&argTargetNo="+strDocNo+"&SAMLart=" + strArtifact;
			...
			/*
			var strUrl = "";
			//strUrl = "IIC100.aspx?nIIC100=1_0611_011";//[開啟子視窗程式]+[?回傳接值物件(必需)]+[&傳入子視窗預設查詢條件]
			strUrl = "Tree_RoleUser.htm";
			fnOpenChildWin(strUrl, "", 230, 410 );
			*/
			//break;
		case "btPreview":
			Page_BlockSubmit = true;
...
function ClientOnLoad()
{
	...
	//1051201 Cloud   1051181 [鐵改局]因公文性質不同調整畫面說明斷段落以及擬定作業事項
	...
	if (document.all.nOrgNickName && document.all.nOrgNickName.value == "RRB")
	{
	    document.all["divDesc"].style.display = "none";//隱藏資訊
	    if (document.all.DocProtity) {
	    	if (document.all.DocProtity.value == "3")//專案性質公文DetailNoData-不存在表示有細項資料，否則顯示原TEXTBOX
	        {
...
//0991028	Howard	0990647	新增CheckAuditMsg()檢查核可時，是否自動加註核可訊息
function CheckAuditMsg()
{
	//若有註冊此欄位時
	if(document.all["H_AutoAuditMsg"])
	{
		var arrAuto = document.all["H_AutoAuditMsg"].value.split(";");
		//若設定為自動加註時，則判斷核可訊息是否為空
		if(arrAuto[0] == "Y")
		{
			//若為空時，則自動加入核可訊息
			if(jf_Trim(document.all["txAuditMsg"].value) == "")
				document.all["txAuditMsg"].value += arrAuto[1];
		}
	}
}
...
function OpenFile(argFileName) {
	...
	var rtnVal = OD.ODT220.AttDownLoad(DownFromPath, DownToPath, document.all.H_txArtifact.value, document.all.H_WS.value, argFileName).value;
	if (rtnVal[1] != "")
		alert(rtnVal[1]);
	else
