...
function jf_OpenSumDocWin(sUrl)
{
    ...
    //0971128	Leslie	一併修正AKI811與AKI812視窗的開啟行為(兩個程式應該是共用一個視窗)
    ...
    //SumDocWin = open(sUrl,"SumComWin","fullscreen=no,Height="+String(window.screen.height-54)+",Width="+String(window.screen.width-8)+",Top=0,Left=0,Scrollbars=yes,titlebar=yes,status=yes,resizable=no");
    //SumDocWin.focus();
    //1131015 Zen 1130941 弱掃Client Dynamic File Inclusion修正
...
function ClientButtonControl(e)
{
    ...
    switch (xObjectName)
    {
        ...
        case "btOpenEdit":
            Page_BlockSubmit = true;
            //1050622 Cloud 1050087 升級二代-改用二代模組
            DownloadDocument();
            break;
...
function jf_ToolBarHandle(event)
{
    ...
    switch (xObjectName)
    {
        ...
        case "btIMAGE1":		// 線上調檔		//1050901	Leslie	升二代，調整線上調檔部分為不限筆數(一律帶回側屜
            ...
            {
                // 2017.07.27   CLOUD   1060645 增加判斷系統參數，公文產生日期小於系統參數公文不可使用線上瀏覽
        ...
        case "btQueryProcess":
            ...
            if (CheckBeforeQueryProcess("NEW"))
            {
                ...
                //	var xUrl = document.all["ODWebPath"].value + "ODI260.aspx?pDocNo="+document.all["lbDOC_NO"].innerText+"&Artifact="+document.all["Artifact"].value;
                //	jf_OpenChildWin(xUrl,"ODI260",750,450);
            }
            else
            {
                //1140609 Zen 1140147 支援顯示歷史公文流程功能
                if (CheckBeforeQueryProcess("OLD"))
                {
...
function CheckBeforeQueryProcess(argCheckType)
{
...
//1050622 Cloud 1050087 升級二代-無用mark
/*
...
//2016.06.22   Cloud   1050087 升級二代-無用MARK-S
/*function OpenUnv()
{
...
function jf_btApply1Click()
{
    ...
    var rtn = AK.AKI812.btAPPLY1Click(jf_GetArtifact(), sDocCheck);
    if (rtn.value)
    {
        var arRtn = rtn.value;
        if (arRtn[3] != "")
