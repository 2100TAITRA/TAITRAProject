...
window.onbeforeunload = function () {
	AK.AKI800_SMEG.SessionClose(jf_GetSessionID());
};
...
function ClientOnLoad() {
	...
	//1090915	Leslie[1090578]	信保基金客製化功能
	$('#txTaxId,#txBankNo,#txBankRNo,#txCCardNoS,#txCCardNoE,#txManageBNoS,#txManageCNoS,#txManageBNoE,#txManageCNoE').on('blur', function () { PADZERO(this, this.maxLength); });
}
...
function Dateonclick(type)//0960827 001474 Matte
{
    ...
    } else
    {
        ...
        var resultE = AK.AKI800_SMEG.CalculateDate('1');
		document.all.tbDATEE.value = resultE.value.Rtn;
        //1070830 Zen 1070678 弱掃Ajax修正
        //var resultS = AKI800.CalculateDate(type);
        var resultS = AK.AKI800_SMEG.CalculateDate(type);
...
function btSearchClientSideProc(argBtName)
{
    ...
    var rtn = AK.AKI800_SMEG.btSaveAjax(jf_GetArtifact(), ParaList, valueList, argBtName);
	if(rtn.error){
