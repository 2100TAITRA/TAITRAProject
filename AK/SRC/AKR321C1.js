...
function Check_Required_Field()
{
    ...
    //1110315 Zen 1101545 (考試院)以AKR321為基底新增AKR321C1
    var strSeqNoS = $('#txSeqNoS').val();
    var strSeqNoE = $('#txSeqNoE').val();

    if (strSeqNoS == '' && strSeqNoE != '')
        $('#txSeqNoS').val(strSeqNoE);
    else if (strSeqNoS != '' && strSeqNoE == '')
        $('#txSeqNoE').val(strSeqNoS);
    else if (Number(strSeqNoS) > (Number(strSeqNoE))) 
    {
        $('#txSeqNoS').val(strSeqNoE);
        $('#txSeqNoE').val(strSeqNoS);
    }

    document.all['tbFILE_VOL1'].value = document.all['tbFILE_VOL2'].value;
    return true;
}
