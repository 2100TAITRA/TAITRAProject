 ...
 * 日期		修改人	單號	概要
 * -------------------------------------------------------------------------------------------------
 * 2006.10.04	Jeff	950938	(併950948,950962) 解決onblur會把併件選項不勾選的問題
 * 2006.10.11	David	950920	直接按下編卷後僅帶案次號未帶案名
 * 2006.10.12	David	950489	增加科別選項UPDATE科別相關欄位,使修改承辦單位後不會無權限調閱影像檔
 * 2006.11.01	David	950869	取消併件行為以及提示訊息
 * 2006.11.16	David	955195	藥檢 - 新增併案資訊按鈕，帶出AKI811顯示併案、件明細
 * 2006.12.04	David	950941	線上簽核編卷滿卷及重複編卷問題
 * 2007.02.26	David	000189	勤益-二級單位顯示異常
 * 2007.03.16	David	960061	台科大EAT220呼叫AKM330修改→自動關閉視窗
 * 2007.05.07   Matte   000291  使用者執行儲存目次號為000文件時，提醒使用者不可為000
 * 2007.05.30	Leo		001007	[職訓局]輸入參考文號時子文的保存年限要和母文一樣
 * 2007.07.31	Leo		001206	密等為普通以上時，應用限制自動帶不開放
 * 2007.08.01	Leo		001209	combobox在回server時，無法將client所改變的值帶回server，所以用隱藏欄位來存承辦人的資訊
 * 2007.08.06	Leo		無單號	[國合會]開啟子文時要提示訊息
 * 2007.08.30	Zoey	001477	增加分類號長度檢查
 * 2007.10.22   Matte   001504  新增併件流水號自動取號
 * 2007.11.06	Cola	--	(MAC610003)AKT116-透過AKM330編目後批次點收，固定有一份以上公文無法正常點收 
 * 2007.11.06	Cola	001273	傳回AKT116後，點收時出現錯誤訊息，修正htxClsKey值邏輯
 * 2007.11.12	Cola	---	依系統參數調整欄位
 * 2007.11.16	Cola	001054	儲存前，若該筆公文已有編目日期，則不允許部份檔號為空的情況下，進行儲存
 * 2007.11.16	Cola	0960193	修正訊息『[國合會]開啟子文時要提示訊息』，須僅於開啟後跳出訊息
 * 2007.11.16	Cola	001600	取消dlRmvSecCode / txKeepSecYear 欄位
 * 2007.11.16	Cola	001600	當應解密期限不為空時，分類號onblur時，不應再依據保存年限帶出應解密期限
 * 2007.11.22	Cola	---	新增於參照文號onblur時必須先判斷有無編目 
 * 2007.11.29	Cola	001209	於將承辦資訊加入combobox前，先行清空，而後先加入一筆空白資料，再加入後續資訊，避免衍生問題 
 * 2007.12.3	Cola	0960162	修改AKM330中分類號查詢子視窗，原本帶出AKC320，現修改為帶出EAC005
 * 2008.2.04	Cola	0960084	修正承辦資訊處理方式 
 * 2008.2.16	Cola	0970153	部份程式碼調整位置，應先Alert再Focus才不會有錯 / 修正隱藏欄位在併案查尋後應先保留住
 * 2008.3.13	Cola	0960446	新增來文字號欄位 
 * 2008.7.30	Cola	0970679	開啟EAC005時，網址參數必需額外傳入SamLart(權仗)
 * 2008.9.04	Cola	0970657	[交通部]新增檔號管理原則
 * 2008.9.22	Cola	0970909	修正儲存時username處理之邏輯
 * 2008.11.07	Cola	0970988	密等與應用限制之連動關係設定
 * 2008.11.26	Leslie	0971045	修正第二次之後進行子文編目併件時，會出現前次處理過的母文檔號
 * 2008.07.27	Howard	0980402	修正AKM330案由欄位檢核以字數來計算
 * 2009.09.16   Jane    0980454 開啟AKM330時多傳入權杖
 * 2009.11.11	Howard	0980587	配合藥檢局四局合一修改依庫房管理模式ChkUserDocPriv()新增公文文號之參數並調整退文訊息
 * 2010.07.05	Leslie	0990308	自動編卷功能，當公文之頁數大於系統設定之最大頁數時，則不依原邏輯取得下一卷之卷、目次號，而已目前之卷、最大目次號回傳，並顯示提示訊息
 * 2010.07.06	Davy	0990366	配合增加辦畢時間欄位，增加檢查時間格式
 * 2011.07.13	Davy	1000129	[檔管局]新增紙本來文併同歸檔相關功能
 * 2011.10.13	Jeff	1000795	修正併件選項無打勾，參照文號輸入後會自動帶入子文檔號
 * 2012.03.07   Cloud   1010201 修正輸入辦畢日期後，未ONBLUR直接按下儲存會造成WORK_DATE出錯的異常BUG
 * 2012.05.25	Cloud	1010369	依領務局要求，新增針對併案、併件關係時提供更動檔號時相關提示功能
 * 2012.05.30	Cloud	------	修正ONBLUR母文異常的錯誤
 * 2013.02.18	Jagle	1011178	領務局：增加目前總頁數欄位
 * 2013.04.08	Jagle	1011178	領務局：目前總頁數欄位要加上隔頁紙數目、該筆公文頁數有調整要連動更新
 * 2013.05.30	Cloud	1000751	修改以機關別稱判斷使用機關
 * 2013.06.21	Jagle	1020406	儲存前先檢核CASE_KEY與案次號是否相符，避免存入錯誤資料
 * 2013.07.24	Cloud	1020126	修正輸入應解密日期，儲存後不會修改的bug
 * 2013.11.07	Cloud	1020885	配合台科大強制點收功能，AKM330用途僅為修改公文基資時，不檢核文件產生日期，並增加公文狀態為非可歸檔時，不可編目
 * 2013.11.26	Cloud	1020885	修正未考量歷史資建檔情況問題-不會有document.all.DocState欄位
 * 2013.12.17	Cloud	1020985	修改保存模式，下拉選單不會連動清除下層值的bug，修正科別選單長度
 * 2014.01.17	Kenny	1030005	調整卷次號及目次號onblur補0的位置，避免輸入完不完整目次號後直接按儲存，造成檔號重複
 * 2014.03.27	Eileen	1030162	新增判斷在(1)clientOnLoad(2)輸入併件文號onblur時，若有勾選併件cbComNo，才將母文的分類號鍵值放至隱藏欄位
 * 2014.09.01	Cloud	--		配合NLB架構，修改開啟子視窗時皆串入權杖
 * 2014.11.12	Kevin_C	1020726	於__doPostBack前加上IsServerHandling=true;避免重複執行
 * 2014.12.04	Gabby	1030888	儲存前將承辦資訊欄位值放入隱藏欄位
 * 2014.12.12	Gabby	--		修正串入權杖時符號使用錯誤與少"&SAMLart="字串等問題
 * 2014.12.26   Cloud   --      修正有開啟領務局檢核併案件功能時，開啟併案公文會出現錯誤的bug
 * 2015.07.07	Kevin_C	1040476	修改文件產生日期取得邏輯
 * 2015.07.29	Cloud	1040629	調整編目時取得編卷效能
 * 2015.08.04	Kevin_C	1040476	判斷長度做切割
 * 2015.11.13	Cloud	1040921	(merge)1020425點選編卷按鈕時增加回傳清理處置資料
 * 2015.11.20	Cloud	1040629	補上會有機關沒案次號需進行一下ONLBUR取得KEY值
 * 2016.09.01   Kenny   1050801 調整取密件另存流水號叫用的webService
 * 2016.11.03   Kenny   1051138 移除檔號補0後檢核邏輯
 * 2016.12.05   Kenny   1051150 修改弱掃Client Potential Code Injection
 * 2016.02.20   Cloud   1051245 新增取消編卷功能
 * 2016.03.20   CLOUD   1050087 升級二代
 * 2017.04.19	Cloud	1060210 增加取得公文點收流程再判斷儲存時是否需要結案日期
 * 2017.09.30	Kevin_C	1060860	修正取母文版本別時，使用的分類號鍵值錯誤的問題，一併修正CHROME點擊Disabled CheckBox無法觸發onblur的問題
 * 2017.11.28	Cloud	1061179	修改密件流水編號檢核邏輯
 * 2018.03.05	Kevin_C	1060353	北榮客製化功能，增加檢核檔號完整性
 * 2018.07.31   Kevin_C 1070716 修正發文檢核邏輯
 * 1070830      Zen     1070678 弱掃Ajax修正
 * 2019.02.26	Cloud	1080186	修正併案後，未正確帶出案名問題
 * 2019.06.17	Cloud	1080473	修正編目時，判斷公文密等大小更新案卷密等
 * 2019.07.11	Cloud	1080559	修正線上瀏覽發生POST_BACK問題
 * 2019.03.06	Cloud	1081109	修改支援版本/年度互轉，修正子視窗回傳值設定不正確問題，調整取得分類號ws皆為getcls、案次號使用checkcasemain
 ...
 * 2025.05.09	Cloud	1140697	修改當檔號完整，分類號為AA或BB時，不可異動分類號欄位
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
function jf_ToolBarHandle(event)
{
	...
	switch (xObjectName)
	{
		...
		case "btSave":
			...
			//1020621	Jagle	[1020406]	CASE_KEY及案次號有值時，增加進行檢核，避免兩者不一致
			if(document.all.htxCaseKey.value != "" && jf_Trim(document.all["tbCASE"].value) != "")
			{
			    //1070830 Zen 1070678 弱掃Ajax修正
			    //var CaseCheckRes = AKM330.CaseCheck(document.all["tbOrgNo"].value, document.all.htxCaseKey.value, document.all["tbCASE"].value).value;
			    var CaseCheckRes = AK.AKM330_SMEG.CaseCheck(document.all["tbOrgNo"].value, document.all.htxCaseKey.value, document.all["tbCASE"].value).value;
				if(!CaseCheckRes)
				{
					TbOnBlur("tbCASE");
					TbOnChange("tbCASE");
			...
			{
				//若有設定過不再修改, 且分類號並沒有修改過, 則不再詢問是否需修改
				//1060718 Cloud 1050087升級二代 ws回傳物件不同配合修改
				//if (!(jf_Trim(document.all["txInpFileDate"].value) != "" && obj.ErrorClass.ErrMessage[0].text.indexOf("細分類") >= 0 && NotModifyCls && OriginVer_Cls == document.all["tbVerNo"].value+document.all["tbCLS"].value))
				...
				{	
					//[0970248]Add by Cola 若有編目日期，且分類號為非最下層分類號之處理, 且分類號並沒有修改過
					//1060718 Cloud 1050087升級二代 ws回傳物件不同配合修改
					//if (jf_Trim(document.all["txInpFileDate"].value) != "" && checkkey.value.ErrorClass.ErrMessage[0].text.indexOf("細分類") >= 0 && !ModifyCls  && OriginVer_Cls == document.all["tbVerNo"].value+document.all["tbCLS"].value)
					...
					{
					//1060718 Cloud 1050087升級二代 ws回傳物件不同配合修改
						//alert(checkkey.value.ErrorClass.ErrMessage[0].text);
						alert(checkkey.value.ErrorClass.ErrMessage[0]);
						Page_BlockSubmit = true;
						break;
					}
				}
				//[0970248]Add by Cola 若有設定過不再修改, 且分類號並沒有修改過, 則設定分類號鍵值
				else
			...
			if (Check_NOT_Allow_Empty_Field())
			{
				if (before_save())
				{
					...
					if (document.all["USE_STOCK"].value == "Y")
					{
					    ...
					    if (AK.AKM330_SMEG.CheckManageMethod(document.all["h_sourceorgno"].value, document.all["tbDOC_NO"].value).value != "0")
						{
							if(!CheckFileValueProcess(1))
								break;						
						}
					...
					if (document.all["tbCOM_NO"].value != "" && document.all["tbDOC_NO"].value != document.all["tbCOM_NO"].value && document.all["cbComNo"].checked == true )
					{
					    ...
					    var strInpDateData = AK.AKM330_SMEG.CheckInpFile(document.all["h_sourceorgno"].value, document.all["tbCOM_NO"].value).value;
						...
						strArrData = AK.AKM330_SMEG.GetFileNo(document.all["h_sourceorgno"].value, document.all["tbCOM_NO"].value).value.split(',');
		...
		case "btClean":
			...
			//1090910	Leslie[1090571]	信保基金客製化功能
			$('#lbCCardName').text("");
			$('#lbTaxIdName').text("");
			$('#lbManageName').text("");
			$('#lbBankName').text("");
...
function ClientOnLoad()
{
	...
	//* 2025.05.09	Cloud	1140697	修改當檔號完整，分類號為AA或BB時，不可異動分類號欄位
	if (document.all.txInpFileDate != "" && (document.all.tbCLS.value == "AA" && document.all.tbCLS.value == "BB")) {
		document.all.tbCLS.disabled = true;
	}
	else
		document.all.tbCLS.disabled = false;
		...
		switch (xObjectName)
		{
			...
			case "btAutoVol":
				...
				if(document.all.cbComNo.checked && document.all.tbCOM_NO.value != "" && document.all.tbCOM_NO.value != document.all.tbDOC_NO.value)
				{
				    ...
				    var tmp = AK.AKM330_SMEG.CheckInpFile(document.all["h_sourceorgno"].value, document.all["tbCOM_NO"].value).value;
						if (jf_Trim(tmp) == "")
						{
...
function wsGetVolSeq()
{
    ...
    RtnObj = jf_CallWS("lib/AK_LIB.asmx", "SMEG_GetMaxVolSeq", false, param2);
...
function OnWSResult(argResult)
{
    ...
    if(argResult.id == iCallID_CLS)  //分類號 onblur
    {
		...
		if(jf_IsWebServiceSuccess_AKM330(argResult))
		{
			...
			// 2013.05.30	Cloud	1000751	修改以機關別稱判斷使用機關
			//if (document.all["tbOrgNo"].value == '354010000J' && document.all["tbKeep_Year"].value != "" ) //Leo 0960530 001007 除了職訓局，一律以分類保存年限帶入公文保存年限
			/*if (document.all["tbOrgNickName"].value == "EVTA" && document.all["tbKeep_Year"].value != "" ) 
				{}
			else
		...
		else
		{
			...
			{	
				//[0970248]Add by Cola 若有編目日期，且分類號為非最下層分類號之處理，且分類號並沒有修改過
				//1060718 Cloud 1050087升級二代 ws回傳物件不同配合修改
				//if (jf_Trim(document.all["txInpFileDate"].value) != "" && obj.ErrorClass.ErrMessage[0].text.indexOf("細分類") >= 0 && !ModifyCls && OriginVer_Cls == document.all["tbVerNo"].value+document.all["tbCLS"].value)
				...
				{	//1060718 Cloud 1050087升級二代 ws回傳物件不同配合修改	
					//alert(obj.ErrorClass.ErrMessage[0].text);
					//if(obj.ErrorClass.ErrMessage[0].text.indexOf("、")<0) //除了不同版本有同分類號情況外一律清除資料 #2006.02.07 Andy
					...
					//[0970248]
					//1060718 Cloud 1050087升級二代 ws回傳物件不同配合修改
					//if(obj.ErrorClass.ErrMessage[0].text.substring(0,3)=="年度號")
					if(obj.ErrorClass.ErrMessage[0].substring(0,3)=="年度號")
					    //1060321 CLOUD 1050087 升級二代
					    //document.all["tbYEAR"].focus();//年度號不正確
					    $("#tbYEAR")[0].focus();//年度號不正確
						//1060718 Cloud 1050087升級二代 ws回傳物件不同配合修改
					//else if (obj.ErrorClass.ErrMessage[0].text.indexOf("、")>=0)
		...
		//alert (IsCls);
		//1020218	Jagle	[1011178]	卷次號為空時要清空已使用總頁數
		if(document.all["tbVOL"].value == "")
			document.all["txVolTotalPage"].value = "";
    }
...
//if(jf_IsWebServiceSuccess(argResult))
			...
			if (WSResult.ErrorClass.ErrMessage.length!=0)  //有err
			{
				...
				document.all["htxCaseKey"].value="";
				//1060718 Cloud 1050087升級二代 ws回傳物件不同配合修改
				//if(WSResult.ErrorClass.ErrMessage[0].text.substring(0,5)=="無此案次號")
				//1090309 Cloud 1081109 配合ws整合修改判斷文字
				//if(WSResult.ErrorClass.ErrMessage[0].substring(0,5)=="無此案次號")
	...
	//d							else
	//d								document.all["tbCASE"].focus();
							} // end if (!IsOpenAKM330C1) 
						}
					}
					else
					{
						//1060718 Cloud 1050087升級二代 ws回傳物件不同配合修改
					    //alert(WSResult.ErrorClass.ErrMessage[0].text);//[0970153]Alter place by Cola 調整位置，應先Alert再Focus才不會有錯
						alert(WSResult.ErrorClass.ErrMessage[0]);//[0970153]Alter place by Cola 調整位置，應先Alert再Focus才不會有錯
					    //1060321 CLOUD 1050087 升級二代
					    //document.all["tbCASE"].focus();						
					    $("#tbCASE")[0].focus();						
					}
...
//		}
    ...
    if(argResult.id == iCallID_Vol)
    {
		if(jf_IsWebServiceSuccess(argResult))
		{
			...
			document.all["tbSEQ"].value = jf_PADL(WSResult.SeqNo, 6, "0");
			...
			// 2013.05.30	Cloud	1000751	修改以機關別稱判斷使用機關
			//if(document.all["tbOrgNo"].value == "A03010000B" && document.all["dlDocFileType"].options[document.all.dlDocFileType.selectedIndex].value == "1")
			if(document.all["tbOrgNickName"].value == "BOCA" && document.all["dlDocFileType"].options[document.all.dlDocFileType.selectedIndex].value == "1")
			{
			    //1070830 Zen 1070678 弱掃Ajax修正
			    //SepPage = AKM330.CountSeq(document.all["tbOrgNo"].value, document.all["tbYEAR"].value, document.all["htxClsKey"].value, document.all["tbCASE"].value, document.all["tbVOL"].value, document.all["tbDOC_NO"].value).value;
			    SepPage = AK.AKM330_SMEG.CountSeq(document.all["tbOrgNo"].value, document.all["tbYEAR"].value, document.all["htxClsKey"].value, document.all["tbCASE"].value, document.all["tbVOL"].value, document.all["tbDOC_NO"].value).value;
				document.all.H_SepPage.value = SepPage;
			}
			if(WSResult.VolPage != "")
			{
			...
			//0990705	Leslie[0990308]	自動編卷功能，當公文之頁數大於系統設定之最大頁數時，則不依原邏輯取得下一卷之卷、目次號，而已目前之卷、最大目次號回傳，並顯示提示訊息
			//1060718 Cloud 1050087升級二代 ws回傳物件不同配合修改
			//if(WSResult.ErrorClass.ErrMessage.length > 0 && WSResult.ErrorClass.ErrMessage[0].text != "")
			...
			{
				//1090309 Cloud 1081109 整合叫用CheckCasemain
				/*var param = new Array(3);
				param[0] = document.all["tbYEAR"].value;
				param[1] = document.all["htxClsKey"].value;
				param[2] = document.all["tbCASE"].value;

				RtnObj = jf_CallWS("lib/AK_LIB.asmx","CheckCase",false,param);*/
				var param = new Array(5);
				param[0] = document.all["tbYEAR"].value;
    ...
    /*if(argResult.id == iCallID_Sys1)
    {
		if(jf_IsWebServiceSuccess(argResult))
		{
			WSResult = argResult.value;
			Num = parseInt(WSResult.RtnField0[0],10);
		}
    }*/
    ...
    */
	...
	//1090910   Leslie[1090571] 信保基金客製化功能
	//專案卡號
	if (argResult.id == iCallID_CCardNo) {
	    if (argResult.error)
	        return;
	    ...
	    if (WSResult.Result != "0") {
	        ...
	        $('#tbCCardName').val(WSResult.ClientName);
	        $('#lbCCardName').text(WSResult.ClientName);
	        if ($('#tbTaxIdNo').val() != WSResult.TxIdNo) {
	            $('#tbTaxIdNo').val(WSResult.TxIdNo);
	            $('#tbTaxIdName').val(WSResult.TxIdName);
	            $('#lbTaxIdName').text(WSResult.TxIdName);
	        }
	    }
	}

	//統一編號
	if (argResult.id == iCallID_TaxId) {
	    if (argResult.error)
	        return;
	    ...
	    if (WSResult.Result != "0") {
	        ...
	        $('#tbTaxIdName').val(WSResult.TxIdName);
	        $('#lbTaxIdName').text(WSResult.TxIdName);
	        if (WSResult.ClientCradNo != ""){
	        	$('#tbCCardNo').val(jf_PADL(WSResult.ClientCradNo,8,"0"));
	            $('#tbCCardName').val(WSResult.ClientName);
	            $('#lbCCardName').text(WSResult.ClientName);
	        }
	    }
	...
	//列管編號
	if (argResult.id == iCallID_ManageCaseNo) {
	    if (argResult.error)
	        return;
	    ...
	    if (WSResult.Result != "0") {
            ...
            //列管案資訊
	        $('#tbManageName').val(WSResult.BankBranchName);
	        $('#lbManageName').text(WSResult.BankBranchName);

            //連動資訊(統一編號、銀行代碼)
	        $('#tbTaxIdNo').val(jf_PADL(WSResult.TxIdNo,8,"0"));
	        $('#tbTaxIdName').val(WSResult.TxIdName);
	        $('#lbTaxIdName').text(WSResult.TxIdName);
	        $('#tbBankCode').val(jf_PADL(WSResult.BankCode,3,"0"));
	        $('#tbBankBranchCode').val(jf_PADL(WSResult.BankBranchCode,4,"0"));
	        $('#tbBankName').val(WSResult.BankName);
	        $('#tbBankBranchName').val(WSResult.BankBranchName);
	        $('#lbBankName').text(WSResult.BankName + "-" + WSResult.BankBranchName);
	    }
	}

	//銀行代碼
	if (argResult.id == iCallID_BankCode) {
	    if (argResult.error)
	        return;
	    ...
	    if (WSResult.Result != "0") {
	        ...
	        $('#tbBankName').val(WSResult.BankName);
	        $('#tbBankBranchName').val(WSResult.BankBranchName);
	        $('#lbBankName').text(WSResult.BankName + (WSResult.BankBranchName ? "-" + WSResult.BankBranchName : ""));
	    }
	}
    //1090910   Leslie[1090571] 信保基金客製化功能   ==END==
...
//var iCallID_Sys = null;
//var iCallID_Sys1 = null;
...
//1090910   Leslie[1090571] 信保基金客製化功能
var iCallID_CCardNo = null; //專案卡號
var iCallID_TaxId = null;   //統一編號
var iCallID_ManageCaseNo = null;    //列管編號
var iCallID_BankCode = null;    //銀行代碼
...
function TbOnBlur(argTextBox)
{
	...
	*/
	...
	if (argTextBox == "tbVOL")
	{
		...
		{
		    //1070830 Zen 1070678 弱掃Ajax修正
		    //var ClearProc = AKM330.GetClearProc(document.all["h_sourceorgno"].value, document.all["tbYEAR"].value, document.all["tbCLS"].value, document.all["tbCASE"].value, document.all["tbVOL"].value);
		    var ClearProc = AK.AKM330_SMEG.GetClearProc(document.all["h_sourceorgno"].value, document.all["tbYEAR"].value, document.all["tbCLS"].value, document.all["tbCASE"].value, document.all["tbVOL"].value);
	...
	if (argTextBox=="txSecSeq")
	{
		...
		var arKeyValue = new Array(3);
		arKeyValue[0] = document.all["tbOrgNo"].value;
		arKeyValue[1] = document.all["txSecSeq"].value;
		arKeyValue[2] = document.all["tbDOC_NO"].value ;
	...
	if (argTextBox=="tbYEAR")
	{
		...
		else
		{
			...
			/*
			//if (document.all["tbCLS"].value!="" && document.all["tbCASE"].value!="")
			if (document.all["tbCLS"].value!="" &&
			document.all["tbCASE"].value != document.all["INITS_FILENO"].value && 
			document.all["tbCASE"].value != document.all["INIT_FILENO"].value  &&
			document.all["tbCASE"].value != document.all["DEPT_CASE"].value )	
			*/
			...
			/*if (!jf_CheckCDATE(document.all["tbYEAR"].value+"0101"))
			{
				alert("年度號格式錯誤");
				document.all["tbYEAR"].focus();
			}*/
	...
	if (argTextBox=="tbCASE")
	{
		...
		if (document.all["tbCASE"].value != document.all["INITS_FILENO"].value && 
			document.all["tbCASE"].value != document.all["INIT_FILENO"].value  &&
			document.all["tbCASE"].value != document.all["DEPT_CASE"].value )	
		//941206 rita if (!IsCls && document.all["tbCLS"].value!="")
		*/
		//改成所有案次號一律去call ws帶案名 #2006.03.19 Andy
		if(jf_Trim(document.all["tbVerNo"].value) != ""  && jf_Trim(document.all["tbCLS"].value) != "")
		{
			//1080309 CLOUD	1081109 整合叫用CheckCaseMain
			/*var param = new Array(3);
			param[0] = document.all["tbYEAR"].value;
			param[1] = document.all["htxClsKey"].value;
			param[2] = document.all["tbCASE"].value;
	...
	if (argTextBox=="tbSEQ")
	{
		if (document.all["tbSEQ"].value=="")
		    ...
		    document.all["tbSEQ"].value = jf_PADL(document.all["tbSEQ"].value, 6, '0');
	}
				...
				if (DeptNo != "")
				{
					...
					{	
						...
						{		
								//依據公文歸檔庫房
						    //1070830 Zen 1070678 弱掃Ajax修正
						    //var StoreName = AKM330.GetStoreName(document.all["h_sourceorgno"].value, document.all["tbCOM_NO"].value).value;
						    var StoreName = AK.AKM330_SMEG.GetStoreName(document.all["h_sourceorgno"].value, document.all["tbCOM_NO"].value).value;
    ...
    //1090910   Leslie[1090571] 信保基金客製化功能
    //專案卡號
	if (argTextBox == "tbCCardNo") {
	    var wsSMEG = $('#h_SMEGWS_URL').val();
	    var objCCardNo = $('#' + argTextBox);
	    var sCardNo = objCCardNo.val();
	    if (jf_Trim(sCardNo) != "") {
	        sCardNol = jf_PADL(sCardNo, 8, "0");
	        objCCardNo.val(sCardNol);
	    }
	    else {
	        $('#tbCCardName').val("");
	        $('#lbCCardName').text("");
	    ...
	    arWSParam[0] = sCardNol;

	    RtnObj = jf_CallW(wsSMEG, "GetCardInfo", false, arWSParam);
	    iCallID_CCardNo = RtnObj.id;
	    OnWSResult(RtnObj);
	}

    //統一編號
	if (argTextBox == "tbTaxIdNo") {
	    var wsSMEG = $('#h_SMEGWS_URL').val();
	    var objTaxIdNo = $('#' + argTextBox);
	    var sTaxIdNo = objTaxIdNo.val();
	    if (jf_Trim(sTaxIdNo) != "") {
	        sTaxIdNo = jf_PADL(sTaxIdNo, 8, "0");
	        objTaxIdNo.val(sTaxIdNo);
	    }
	    else {
	        $('#tbTaxIdName').val("");
	        $('#lbTaxIdName').text("");
	    ...
	    var arWSParam = new Array(1);
	    arWSParam[0] = sTaxIdNo;

	    RtnObj = jf_CallW(wsSMEG, "GetTaxIdInfo", false, arWSParam);
	    iCallID_TaxId = RtnObj.id;
	    OnWSResult(RtnObj);
	}

    //列管編號(二個欄位都有值才做事)
	if (argTextBox == "tbManageBankNo") {
	    if (jf_Trim($('#' + argTextBox).val()) == "")
	        $('#tbManageCaseNo').val("");
	    else {
	        let s = $('#' + argTextBox).val();
	        s = jf_PADL(s, 3, "0");
	        $('#' + argTextBox).val(s)
	    }
	}
	if (argTextBox == "tbManageCaseNo") {
	    var wsSMEG = $('#h_SMEGWS_URL').val();
	    var objManageCaseNo = $('#' + argTextBox);
	    var sManageCaseNo = objManageCaseNo.val();
	    if (jf_Trim(sManageCaseNo) != "") {
	        sManageCaseNo = jf_PADL(sManageCaseNo, 6, "0");
	        objManageCaseNo.val(sManageCaseNo);
	    }
	    else {
	        $('#tbTaxIdName').val("");
	        $('#lbTaxIdName').text("");
	        $('#tbBankCode').val("");
	        $('#tbBankBranchCode').val("");
	        $('#tbBankName').val("");
	        $('#tbBankBranchName').val("");
	        $('#lbBankName').text("");
	        return;
	    }

	    var arWSParam = new Array(2);
	    arWSParam[0] = $('#tbManageBankNo').val();
	    arWSParam[1] = sManageCaseNo;

	    RtnObj = jf_CallW(wsSMEG, "GetManageInfo", false, arWSParam);
	    iCallID_ManageCaseNo = RtnObj.id;
	    OnWSResult(RtnObj);
	}

    //銀行代碼
	if (argTextBox == "tbBankCode" || argTextBox == "tbBankBranchCode") {
	    var wsSMEG = $('#h_SMEGWS_URL').val();
	    var objBankCode = $('#tbBankCode');
	    var objBranchCode = $('#tbBankBranchCode');
	    var sBankCode = objBankCode.val();
	    var sBranchCode = objBranchCode.val();
	    if (jf_Trim(sBankCode) != "") {
	        sBankCode = jf_PADL(sBankCode, 3, "0");
	        objBankCode.val(sBankCode);
	    }
	    else {
	        objBranchCode.val("");
	        $('#tbBankName').val("");
	    ...
	    if (jf_Trim(sBranchCode) != "") {
	        sBranchCode = jf_PADL(sBranchCode, 4, "0");
	        objBranchCode.val(sBranchCode);
	    }

	    var arWSParam = new Array(2);
	    arWSParam[0] = sBankCode;
	    arWSParam[1] = sBranchCode;

	    RtnObj = jf_CallW(wsSMEG, "GetBankInfo", false, arWSParam);
	    iCallID_BankCode = RtnObj.id;
	    OnWSResult(RtnObj);
	}
	...
	if (argCheckBox=="cbComNo")
	{
		if(document.all[xObjectName].checked)
		{
			...
			if(document.all.dlDocFileType.options[document.all.dlDocFileType.selectedIndex].value != document.all.htxDocFileType.value) 
			{
			    ...
			    if (jf_Trim(AK.AKM330_SMEG.CheckInpFile(document.all["h_sourceorgno"].value, document.all["tbCOM_NO"].value).value) != "")
			    {
					if(document.all.tbYEAR.value == "" && document.all.tbCLS.value == "" && document.all.tbCASE.value == "" && document.all.tbVOL.value == "" && document.all.tbSEQ.value == "")
					{
						document.all.tbYEAR.value = document.all.htxFileYear.value;
...
function ChkEFileVolNo()
{
	if (document.all["tbVOL"].value!="")
	{
		if(document.all.dlDocFileType.options[document.all.dlDocFileType.selectedIndex].value =="2") //電子檔案
		{
			//1091202	Leslie	信保電子檔案，不檢核是否編列紙本卷
...
function GetFileComBoSeq()
{
	...
	else
	{
        ...
        var resultE = AK.AKM330_SMEG.GetFileComSeq(document.all["h_sourceorgno"].value, document.all["tbCOM_NO"].value);
		if((jf_Trim(document.all["tbCOM_NO"].value) !="") ) 
		{
