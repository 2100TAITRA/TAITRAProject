...
function jf_ToolBarHandle(event)
{
	...
	switch (xObjectName)
	{
		case "btClean":
		    Page_BlockSubmit = true;
		    var TempOrg = document.all.H_OrgNO.value;
		    var TempSet =  document.all["AKR320_SUPPORT_CLSE_KEYIN"].value
		    if (jf_ConfirmClean()) {
		        document.all.H_OrgNO.value = TempOrg;
		        document.all["AKR320_SUPPORT_CLSE_KEYIN"].value = TempSet;
		        ...
		        document.all["cbLine"].checked = true;
		        document.all["cbShowPage"].checked = true;
...
function fNSetFileNo()
{
    if (document.all["txDocNo"].value != "") {
        var RtnValue = AK.AKR320_MOCS.GetDocFileNo(document.all["H_OrgNO"].value, document.all["txDocNo"].value).value;
        if (RtnValue.indexOf("ERR-") != -1)
        { alert(RtnValue); }
        else
        {
            var arrFileIfno = RtnValue.split('|');
            document.all["tbVerNo"].value = arrFileIfno[0];
            document.all["tbFILE_YEAR"].value = arrFileIfno[1];
            document.all["tbFILE_CLS1"].value = arrFileIfno[2];
            document.all["tbFILE_CLS2"].value = arrFileIfno[2];
            document.all["tbFILE_CASE1"].value = arrFileIfno[3];
            document.all["tbFILE_CASE2"].value = arrFileIfno[3];
            document.all["tbFILE_VOL1"].value = arrFileIfno[4];
            document.all["tbFILE_VOL2"].value = arrFileIfno[4];
            document.all["lbFILE_CLS1"].textContent = arrFileIfno[5];
