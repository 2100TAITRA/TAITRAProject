...
function jf_ToolBarHandle(e)
{
    ...
    switch (xObjectName)
    {
        ...
        //    break;
        case "btSave":
            if (document.all("tbORG_NO").disabled == false)
            {
                if (jf_Trim(document.all("tbORG_NO").value) == "")
                {
                    alert("機關代碼不可為空白");
                    Page_BlockSubmit = true
                }
            }
            if (jf_Trim(document.all("tbORG_NAME").value) == "")
            {
                alert("機關名稱不可為空白");
                Page_BlockSubmit = true
            }
            ...
            break;
        case "btDelete":
            if (jf_Trim(document.all["tbORG_NO2"].value) == "")
            {
                alert("欲刪除機關團體代碼不可空白!!");
                //1070815 Zen 1070073 二代升級
                //document.all["tbORG_NO2"].focus();
                $('#tbORG_NO2').focus();
                Page_BlockSubmit = true;
            }
            else
...
{
    var pORG_NO, pORG_NAME, pCount;
    pCount = parseInt(document.all("lbTemp").options[0].value) + 1;
    //1070815 Zen 1070073 二代升級
    //pORG_NO = document.activeElement.innerText;
    //1100204 Zen 1090927 取消使用document.activeElement    //pORG_NO = document.activeElement.textContent;
    pORG_NO = RetFieldDesc;
    for (var i = 1; i < pCount; i++)
    {
        if (pORG_NO == document.all("lbTemp").options[i].value)
            //1070815 Zen 1070073 二代升級
            //pORG_NAME = document.all("lbTemp").options[i].innerText;
            pORG_NAME = document.all("lbTemp").options[i].textContent;
    }


    opener.document.all(document.all("_rtnObj").value).length = 4;
    opener.document.all(document.all("_rtnObj").value).options[0].text = "ORG_NO";
    //1070815 Zen 1070073 二代升級
    ...
    opener.document.all(document.all("_rtnObj").value).options[1].text = "ORG_NAME"
    opener.document.all(document.all("_rtnObj").value).options[1].value = pORG_NAME;


    opener.window.CallBack("SYC280");
