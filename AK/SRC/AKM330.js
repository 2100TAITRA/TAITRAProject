 ...
 * 2020.11.16	Cloud	1090792	配合法規調整保存狀況代碼，新增已遺失註記
 ...
 * 2021.08.17	Cloud	1100873	修正儲存後密件公文應解密日期一直被重設問題 
 * 2021.09.01	Cloud	1100871	修正文件產生日檢核邏輯不同及未依法規帶出文件產生日問題
 * 2021.10.07	Cloud	1100873	修正案次號DoubleCheck邏輯錯誤問題-原是因ONBLUR及儲存前都會進行檢核，但現行會先比對隱藏欄位舊值與畫面值才進行，故應不會在觸發雙重檢核-故MARK
 * 2021.12.02	Cloud	1101383	參照AKI800，提供併案子視窗線上瀏覽功能
 * 1110103		Zen     1101292	修正多次點擊重複PostBack之問題
 * 2022.01.17 	Cloud 	1110039 修正文號欄位未trim空白問題
 * 2022.02.15   Cloud   1101485 修改支援紙本公文線上瀏覽
 * 2022.06.02   Cloud   1110371 配合純檔管系統修改線上瀏覽webfilio url取得方式
 * 2022.06.09   Cloud   1110520 補強紙本公文編卷僅能用純數字
 * 2022.06.28   Cloud   1110371 交通部序3 修改分類號未異動onblur時協助帶出年度號、如異動後onblur不跳提示訊息，配合修改純檔管線上瀏覽行為
 * 2022.08.11   Cloud   1110414、1110418 提供可維護原始單位以及密件併件子文不可自行取得密件流水號
 ...
 * 2022.09.07   Cloud   1110418 密件併件子文，儲存不檢核流水號
 * 2022.10.26   Cloud   1111061 修正應解密日期有值時，不該再被異動，1111277 參照文號不存在或未歸檔時，不應可設定
 * 2022.12.19   Cloud   1111167 修正版本別與分類號無窮迴圈檢核問題
 * 2023.04.13   Cloud   考試院序7、34 來發文字號增加以tootip顯示完整字號
 * 2023.11.09   Cloud   領務局序91 取消線上立案表示不需要，清空案次號
 * 2024.07.03	Cloud   1130503 修正誤用母文案次號鍵值取代子文問題
 * 2024.07.29	Cloud	1130696	修改開啟AKM336時增加串入庫房代碼、簽核類型
 * 2024.09.30	Cloud	北榮序88	修正啟用開啟時自動檢查併件母子文功能時會跳出異常問題
 * 2025.02.06	Cloud	1131337、1131054	增加國教院使用時，不自動帶入應解密日期且儲存時密件需檢核必須輸入該日期；增加紀錄單位、科別有異動時才做下層連動
 * 2025.07.21	Cloud	1140784	修改支援維護本來文併同歸檔欄位
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
var strComClsKey = "";
//* 2025.02.06	Cloud	1131054	增加國教院使用時，不自動帶入應解密日期且儲存時密件需檢核必須輸入該日期
var bISNaer = false;
...
//* 2021.05.06   Cloud   1090580 AKM330有開放檔管人員可維護保存年限，但1090580增加儲存前的ONBLUR後，造成每次儲存就恢復預設值問題
//* 2021.08.17	Cloud	1100873	修正儲存後密件公文應解密日期一直被重設問題-就算是儲存前觸發 也不該僅取鑑值
...
function jf_ToolBarHandle(event)
{
	...
	switch (xObjectName) {
	    ...
	    case "btSave":
	        ...
	        //1110811 Cloud 1110414 增加判斷910101之後公文異動承辦單位(原始)時跳出提醒-E
	        //1090902 Zen 1090580 修正異動檔號後未onblur直接點擊儲存時，可略過檢核寫錯誤資料至[FILE_MAIN]問題
	        //* 2021.05.06   Cloud   1090580 AKM330有開放檔管人員可維護保存年限，但1090580增加儲存前的ONBLUR後，造成每次儲存就恢復預設值問題
	        bGetKeyBeforeSave = true;
	        //2021.08.18	Cloud	1100873 修正[1090580]修正方式-檔號有異動時才需觸發onblur取得鍵值及相關設定
	        /*TbOnBlur("tbVerNo");
            TbOnBlur("tbYEAR");
            TbOnBlur("tbCLS");
            TbOnBlur("tbCASE");*/
	        //1100901 Cloud -補強，前值為空白時，不做變動檢核-避免回溯建檔一直跳
	        //if (document.all.OtxFullFileNo.value != document.all.tbVerNo.value + "|" + document.all.tbYEAR.value + "|" + document.all.tbCLS.value + "|"
	        ...
	        if (document.all.htxCaseKey.value != "" && jf_Trim(document.all["tbCASE"].value) != "") {
	            ...
	            var CaseCheckRes = AK.AKM330.CaseCheck(document.all["tbOrgNo"].value, document.all.htxCaseKey.value, document.all["tbCASE"].value).value;
	            if (!CaseCheckRes) {
	                TbOnBlur("tbCASE");
	                TbOnChange("tbCASE");
	                Page_BlockSubmit = true;
	        ...
	        if (Check_NOT_Allow_Empty_Field()) {
	            if (before_save()) {
	                ...
	                if (document.all["USE_STOCK"].value == "Y") {
	                    ...
	                    if (AK.AKM330.CheckManageMethod(document.all["h_sourceorgno"].value, document.all["tbDOC_NO"].value).value != "0") {
	                        if (!CheckFileValueProcess(1))
	                            break;
	                    }
	                }
	                ...
	                if (document.all["tbCOM_NO"].value != "" && document.all["tbDOC_NO"].value != document.all["tbCOM_NO"].value && document.all["cbComNo"].checked == true) {
	                    ...
	                    var strInpDateData = AK.AKM330.CheckInpFile(document.all["h_sourceorgno"].value, document.all["tbCOM_NO"].value).value;
	                    ...
	                    strArrData = AK.AKM330.GetFileNo(document.all["h_sourceorgno"].value, document.all["tbCOM_NO"].value).value.split(',');
					...
					*/
					...
					//* 2025.02.06	Cloud	1131054	增加國教院使用時檢核應解密日期必須輸入
	        ...
	        //if (document.all["tbOrgNickName"].value == "TVGH") {
	        	//1111026 Cloud 1111206 陸委會有轉入公文無法決定權責單位，提供參數設定不檢核
	    ...
	    case "btViewDoc":
	        ...
	        if (document.all.USE_T2100_OD.value == "0") {
	            var RtnValue = AK.AKM330.ViewDoc(document.all["h_sourceorgno"].value, document.all.txhUser.value, document.all["tbDOC_NO"].value).value;
		...
		switch (xObjectName)
		{
			...
			case "btAutoVol":
				...
				if(document.all.cbComNo.checked && document.all.tbCOM_NO.value != "" && document.all.tbCOM_NO.value != document.all.tbDOC_NO.value)
				{
				    ...
				    var tmp = AK.AKM330.CheckInpFile(document.all["h_sourceorgno"].value, document.all["tbCOM_NO"].value).value;
						if (jf_Trim(tmp) == "")
						{
			...
			case "btATT_INFO"://附件資訊 AKM336
				...
				//* 2024.07.29	Cloud	[1130696]修改開啟AKM336時增加串入庫房代碼、簽核類型-使用h_signType切換的時候不會改變，一併調整使用strSingType
				//pUrl = "AKM336.aspx?rtnObj=SubWinRtn&m=p&k1="+document.all["tbDOC_NO"].value+"&k2="+document.all["tbCrtDate"].value+"&k3="+document.all["h_signType"].value+"&SAMLart="+GetParam("SAMLart");
...
function OnWSResult(argResult)
{
    ...
    {
    	//1100818 Cloud 1100873-判斷是否重複檢核-s
    	//* 2021.10.07	Cloud	1100873	修正案次號DoubleCheck邏輯錯誤問題
    	/*if (bDubCheck)
    	{
    		bDubCheck = false;
    		return;
    	...
    	bDubCheck = true;*/
    	//1100818 Cloud 1100873-判斷是否重複檢核-e
		IsCls=false;
		if (jf_IsWebServiceSuccess_AKM330(argResult))
		{
        	...
        	WSResult = argResult.value;
        	//* 2021.08.17	Cloud	1100873	修正儲存後密件公文應解密日期一直被重設問題-就算是儲存前觸發 也不該僅取鑑值
        	//if (!bGetKeyBeforeSave)//非儲存前檢核則為原邏輯
        	//{
        		...
        		//1100817 Cloud 1100873 判斷當前畫面跟隱藏欄位版本+分類是否相同，不同表示分類號變更須設定預設值
				var arrFileNo = document.all.OtxFullFileNo.value.split('|');
        		...
        		if (document.all["tbVerNo"].value + "|" + document.all["tbCLS"].value != arrFileNo[0] + "|" + arrFileNo[2]) {
        		    ...
        		    if (document.all["tbCrtDate"].value != "") {
        		        ...
        		        if (!document.all["tbExtRmvSec_Date"].disabled) {
        		            ...
        		            else
        		            {
								...
								//* 2025.02.06	Cloud	1131054	增加國教院使用時，不自動帶入應解密日期
        		                //if (document.all["tbExtRmvSec_Date"].value == "") {
        		    ...
        		    //* 2022.06.28   Cloud   1110371 交通部序3 修改分類號未異動onblur時協助帶出年度號、如異動後onblur不跳提示訊息
			...
			{
				//儲存前檢核則單原本想處理問題，僅設定KEY值
				document.all["htxClsKey"].value = WSResult.CLS_KEY;
			}*/
		}
		else
		{
			//2021.08.18 CLOUD 1100873-儲存前檢核未過
        	bGetKeyBeforeSave = false;
			//若有設定過不再修改, 且分類號並沒有修改過, 則不再詢問是否需修改
			//1060718 Cloud 1050087升級二代 ws回傳物件不同配合修改
			//if (!(jf_Trim(document.all["txInpFileDate"].value) != "" && obj.ErrorClass.ErrMessage[0].text.indexOf("細分類") >= 0 && NotModifyCls && OriginVer_Cls == document.all["tbVerNo"].value+document.all["tbCLS"].value))
...
//		}
    ...
    //1121109   Cloud 沒有地方叫CASE1了 MARK 好看code
    /*
    ...
    if(argResult.id == iCallID_ComNo)
    {
		if(jf_IsWebServiceSuccess(argResult))
		{
			...
			else
			{
			    ...
			    //*2022.10.26   Cloud   1111277 參照文號增加檢核是否已歸檔
    ...
    if(argResult.id == iCallID_ComNo1)
    {
		if(jf_IsWebServiceSuccess(argResult))
		{
			...
			*/
			...
			if (document.all["OrgNickName"].value == "EXAM") {
			    var strArrData = AK.AKM330.GetFileNo(document.all["h_sourceorgno"].value, document.all["tbCOM_NO"].value).value.split(',');
			    document.all.hMStoreDate.value = strArrData[15];
			}
    ...
    if(argResult.id == iCallID_Vol)
    {
		if(jf_IsWebServiceSuccess(argResult))
		{
			...
			if(document.all["tbOrgNickName"].value == "BOCA" && document.all["dlDocFileType"].options[document.all.dlDocFileType.selectedIndex].value == "1")
			{
			    ...
			    SepPage = AK.AKM330.CountSeq(document.all["tbOrgNo"].value, document.all["tbYEAR"].value, document.all["htxClsKey"].value, document.all["tbCASE"].value, document.all["tbVOL"].value, document.all["tbDOC_NO"].value).value;
    ...
    */
    ...
    if(argResult.id == iCallID_SecSeq)
    {
    	...
    	else
    	{
		    ...
		    //1110907 Cloud 1110418 併件密件子文不檢核-E
		    //1110907 Cloud 1110418 併件密件母文，需取得子文出來比對
		    var SunDocList = "";
		    if (document.all["tbCOM_NO"].value != "" && document.all["tbCOM_NO"].value == document.all["tbDOC_NO"].value && document.all["cbComNo"].checked)
		    {
		        SunDocList = AK.AKM330.GetSunDoc(document.all["h_sourceorgno"].value, document.all["tbDOC_NO"].value).value;
		    }
    		for (iDoc = 0; iDoc < RtnObj.value.RtnField0.length; iDoc++)
    		{
...
function TbOnChange(argTextBox)
{
	...
	switch(argTextBox)
	{
		...
		case "txKeepSecYear":
		    ...
		    //* 2022.10.26   Cloud   1111061 修正應解密日期有值時，不該再被異動
			//* 2025.02.06	Cloud	1131054	增加國教院使用時，不自動帶入應解密日期
			//if (document.all.tbExtRmvSec_Date.value=="")
			if (document.all.tbExtRmvSec_Date.value == "" && !bISNaer)
	...
	*/    
	...
	if (argTextBox == "tbVOL")
	{
		...
		else if(document.all["tbYEAR"].value != "" && document.all["tbCLS"].value != "" && document.all["tbCASE"].value != "" && document.all["tbVOL"].value != "")
		{
		    ...
		    var ClearProc = AK.AKM330.GetClearProc(document.all["h_sourceorgno"].value, document.all["tbYEAR"].value, document.all["tbCLS"].value, document.all["tbCASE"].value, document.all["tbVOL"].value);
				...
				if (DeptNo != "")
				{
					...
					{	
						...
						{		
						    ...
						    var StoreName = AK.AKM330.GetStoreName(document.all["h_sourceorgno"].value, document.all["tbCOM_NO"].value).value;
...
function CbOnClick(argCheckBox)
{
	...
	//if (document.activeElement != null)
    //	xObjectName = document.activeElement.id;
	//1130930	CLOUD	北榮序88 修正啟用開啟時自動檢查併件母子文功能時會跳出異常問題
	...
	if (argCheckBox=="cbComNo")
	{
		if(document.all[xObjectName].checked)
		{
			...
			if(document.all.dlDocFileType.options[document.all.dlDocFileType.selectedIndex].value != document.all.htxDocFileType.value) 
			{
			    ...
			    if (jf_Trim(AK.AKM330.CheckInpFile(document.all["h_sourceorgno"].value, document.all["tbCOM_NO"].value).value) != "")
			    {
					if(document.all.tbYEAR.value == "" && document.all.tbCLS.value == "" && document.all.tbCASE.value == "" && document.all.tbVOL.value == "" && document.all.tbSEQ.value == "")
					{
						document.all.tbYEAR.value = document.all.htxFileYear.value;
...
function GetFileComBoSeq()
{
	...
	else
	{
        ...
        var resultE = AK.AKM330.GetFileComSeq(document.all["h_sourceorgno"].value, document.all["tbCOM_NO"].value);
		if((jf_Trim(document.all["tbCOM_NO"].value) !="") ) 
		{
