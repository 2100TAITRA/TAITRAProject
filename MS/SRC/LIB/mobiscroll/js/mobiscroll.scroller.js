    ...
    $.mobiscroll.classes.Scroller = function (elem, settings) {
        ...
        function hide(prevAnim) {
            ...
            if (activeElm && !prevAnim) {
                setTimeout(function () {
                    preventShow = true;
                    // 2019.10.4 - 1080339 Eric, jQuery 3.0 upgrade .focus() -> .trigger('focus')
        ...
        that.position = function (check) {
            ...
            if (!(ww === nw && wh === nh && check) && !preventPos && (event('onPosition', [dw, nw, nh]) !== false) && isModal) {
                ...
                if (/modal|bubble/.test(s.display)) {
                    ...
                    totalw += 2; // 2016.10.13 - Eric Peng, add 2 pixels to prevent string ellipse ending
        ...
        */
        that.setValue = function (values, fill, time, temp, change) {
            that.temp = Array.isArray(values) ? values.slice(0) : s.parseValue.call(e, values + '', that);
