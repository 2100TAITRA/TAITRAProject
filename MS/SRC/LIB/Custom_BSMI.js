...
/*
Date	SA		PG		NO			Desc
1120419	David	David	-------		標檢局二代升級
1130119	David	David	1120995		新增案次號檢核
1140325	David	David	1131242		新增案次號查詢
1140815	Raymond	Raymond	1141232		合併1130313, 離線版改用專用查詢分類號(EAH005.html)視窗
*/

(function() {	
	theCustom.register(CUSTOM.LOAD, function(dm) 
	{
		console.log("[BSMI客製化] 載入文稿完成...");
		var dec;
		...
		.register(CUSTOM.CHANGE, "案次號",Common.fnCheckCase)//1130119 David 1120995 新增案次號檢核
		.register
		(CUSTOM.REFRESH, "發文機關", 
			[	// 發文機關異動時連動以下欄位
	...
	theCustom.register(CUSTOM.SHOWDLG, "分類號", "查詢", function(dm) {
		...
		// 1140815 Raymond 1141232 合併1130313, 離線版改用專用視窗
		if(theSSO.offlineMode)
			return "/ms/EAH005.html"+strQry;
    ...
    if(window.theModMgr != undefined)
        window.theModMgr.install("Custom_BSMI.js").finish();
})();