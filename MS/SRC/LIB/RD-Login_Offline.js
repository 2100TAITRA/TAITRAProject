...
(function($){
	...
	theSSO.offlineMode = true;	// 1130809 Raymond 1130313 合併1111007(1100394), 設立旗標表示目前執行於離線模式中
	...
	theSSO.MP.loginByCert = function loginByCert(org_id, pincode, finish_callback) {
		...
		.then(function(rslt) {
            ...
            try {
                ...
                window.theWebServices.authws.logonByCert(rslt.certb64, rslt.signature, rslt.timeStamp, org_id, {async: true})	// 1130809 Raymond 1130313 合併1111007(1100394), 離線版要改用非同步
                .then(function(rslt) {
					if (typeof SSO_CONFIG.debugTime==='boolean' && SSO_CONFIG.debugTime===true) {
...
$(function() {
	 ...
	 */
	// 1130809 Raymond 1130313 合併1111007(1100394), 離線版只顯示最後一次連線登入更新資源檔的機關
	//function _initOrgListSpinWheel(id, addAll, display_pos) {
	function _initOrgListSpinWheel(id, addAll, display_pos, orgNo) {
	...
	_initControls();
	SSO_CONFIG.EDocPage = "eDoc_Offline.html";	// 1130809 Raymond 1130313 合併1111007(1100394), 改開啟離線版主頁
    ...
    $('#btn_login').on('click', function() {
		...
		if($('#radio-choice-loginbyAccount')[0].checked) {
			...
			if (typeof orgid === 'string' && typeof userid === 'string' /*&& typeof userKey === 'string'*/) {	// 1130809 Raymond 1130313 合併1111007(1100394), 離線模式不檢核密碼
				if (orgid.length>0 && userid.length>0 /*&& userKey.length>0*/) {	// 1130809 Raymond 1130313 合併1111007(1100394), 離線模式不檢核密碼
					
					if(!theSSO.offlineMode) {	// 1130809 Raymond 1130313 合併1111007(1100394), 離線模式不要check
					...
					if (_debugLogin) { // for none-WebService testing
						...
						setTimeout(function() {
							// 1130809 Raymond 1130313 合併1111007, 沒有_t, 離線版網頁開不了...
                            //window.location.href = SSO_CONFIG.EDocPage+'?='+Date.now();
							window.location.href = SSO_CONFIG.EDocPage+'?_t='+Date.now();
                        }, 100);	
					}
					else {
						window.scrollTo(0, 0);
			...
			if(!!navigator.serviceWorker.controller &&
				...
				navigator.serviceWorker.ready.then(function(registration) {
					...
					registration.addEventListener('updatefound', function() {
						...
						$('#btn_login').closest(".ui-btn").addClass("ui-disabled");
						$('#btn_login').val('更新離線版公文製作').button("refresh");
						newInstall.postMessage("offlineMode=true");	// 若已安裝過離線版ServiceWorker, 離線模式登入頁要先通知ServiceWorker預設為離線模式
						// 偵測安裝結束
						newInstall.addEventListener('statechange', function(evt) {
							console.log("ServiceWorker.onstatechange", evt.target.state);
							if(evt.target.state == "activated") {
								$('#btn_login').closest(".ui-btn").removeClass("ui-disabled");
								$('#btn_login').val('登　　入').button("refresh");
				...
				$('#btn_login').closest(".ui-btn").addClass("ui-disabled");
				alert("尚未安裝離線版公文製作，請先以連線版登入公文系統，\r\n再點擊首頁右上角之「選單」按鈕，於選單子視窗中\r\n點擊「安裝離線版公文製作」按鈕進行安裝。");
			}
			// 安裝/更新進度條及取得最後一次連線登入更新資源檔的機關代碼
			navigator.serviceWorker.addEventListener('message', function(evt) {
				console.log(evt.data);
				if(typeof evt.data == "string") {
					if(evt.data.match(/^lastUpdatedOrgRsrc=/)) {
						_initOrgListSpinWheel("orgid_input", false, 'bottom', evt.data.substr(19));// 離線版只顯示最後一次連線登入更新資源檔的機關
					}
						...
						if(param.action == "install") {
							...
							$('#btn_login').val('更新離線版公文製作(' + param.downloadedFiles + '/' + param.totalFiles + ')').button("refresh");
						}
			...
			function _callChangeUserKey(userId, oldKey, newKey, finish_callback) {
				...
				promise.done(function(rslt) {
					if (rslt.indexOf('ERR-')===-1)
					{
						...
						/* 1120327 Raymond 1111007 合併1100394, 離線版不要
						//1070831	Leslie	配合滲透測試修改，登入後直接把Artifact寫入Cookie
						$.ajax({
							type:"POST",
							url:SSO_CONFIG.ServerHost+"/iiws/registCookie.ashx?ck="+SAMLart,
