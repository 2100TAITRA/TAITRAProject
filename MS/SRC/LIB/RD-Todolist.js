...
1131121	1130552		Leslie	Leslie	[併1130877]修正MP主旨重覆執行編碼後，造成的顯示異常
...
1131217	序429		Joe		Joe		修正參數數量傳錯導致無法正確判斷的問題
1140102	序1276		Joe		Joe		修正已走過的一層流程應保留在最前方
...
1141030 序333		David	David	修正會稿單位設定連動預排流程處理，支援自行輸入的會辦單位判斷
1141216	序447		David	David	會稿單位設定連動預排流程時，排除一層決行單位(會稿單位選擇一層決行單位是為了簽核框要有，不是真的要順會)
1150109	1141423		Leslie	Leslie	修正MP欄位[目前位置]應依循「FOLDER_DISCUSS、SUBFOLDER_DISCUSS_HOST」的設定而顯示
...
(function($){
	...
	if (!!theWebServices) {
	    ...
	    theWebServices.odmssp = {
			...
			getDocToDoList2: function (artifact, sourceOrgNo, docNo, options)
	        {
	            //var _dfd = $.Deferred();
	            var wsFuncName = 'GetDocTodoList2';
	            ...
	            var params = new SOAPClientParameters(), res;
	            params.add('argArtifact', artifact);
                params.add('argOrgNo', sourceOrgNo);
	            params.add('argDocNo', docNo);
			...
			SendTimeOutNotify: function(artifact, docList, options){
				var wsFuncName = 'SendTimeOutNotify';
	            var wsUrl = (options && options.url) ? options.url : theWebServices.url('odmsspws');
	            if (!wsUrl || wsUrl.length === 0) {
	                theLogger.error('-E- odmssp.' + wsFuncName + ' was invoked, but WS\'s url was missing!');
	                return { success: false, errCode: -1000, errMsg: 'odmssp WS尚未設定服務網址URL' };
			...
			GetDocTodoListTarget: function (artifact, sourceOrgNo, docNo, userName, ouId, type, options) {
				options = (typeof options=='object')?options:null;
	            ...
	            var wsFuncName = 'GetDocFlowTarget';
	            var wsUrl = (!!options && options.url) ? options.url : theWebServices.url('odmsspws');
	            if (!wsUrl || wsUrl.length === 0) {
	                theLogger.error('-E- ODMSSP.' + wsFuncName + ' was invoked, but WS\'s url was missing!');
	                _dfd.reject({success:false, _errMsg: 'ODMSSP WS尚未設定服務網址URL'});
					return _dfd.promise();
	            }

	            if (artifact.length===0 || sourceOrgNo.length===0 || docNo.length===0) {
					var sParam = '';
					if (artifact.length===0) sParam = 'argArtifact';
					if (sourceOrgNo.length===0) sParam += 'argOrgNo';
					if (docNo.length===0) sParam += 'argDocNo';
	                _dfd.reject({success:false, _errMsg:'叫用ODMSSP.' + wsFuncName + ', 無效的參數值:' + sParam});
					return _dfd.promise();
	            }
	            ...
	            params.add('argOrgNo', sourceOrgNo);
				params.add('argDocNo', docNo);
				params.add('argChargeUserName', userName);
				params.add('argOuId', ouId);
	            ...
	            SOAPClient.invoke(wsUrl, wsFuncName, params, fAsync,
					function (rslt) {
						...
						if (typeof rslt === 'object') {
							...
							if (rslt.m_bSuccess === true) {
								res.success = true;
								res.docTodoTarget = rslt.m_docToDoList;
								res.errMsg = '';
								_dfd.resolve(res);
								_theRslt = res;
							}
							...
							var err = {success:false, _errMsg:'叫用ODMSSP.' + wsFuncName + ' 時發生錯誤!'};
							_dfd.reject(err);
							_theRslt = err;
						}
				});
		...
		theWebServices.webFileIO = {
			...
			updateEnvelope : function(artifact, docNo, msgId, workFilename, flags, orgNo, pincode, options) {
				...
				if(theSSO.User.SystemSets.get('DOCSIGN_BY_WORKSTATION') == 'Y'){
					var wksObj = theWebServices.webFileIO.ws_GetWebSfolder(wsUrl,true);
					flags = (SSOUtil.isValueTrue(theSSO.User.EnvSettings.get('II_SUBMIT_SIGN')))?flags:Number(flags)+0x1000;		//通知wksUpdateSignFolder()執行完即主動上傳檔案
					wsFuncName = "wksUpdateSignFolder";
					params = new SOAPClientParameters();
					params.add('argArtifact', artifact);
					params.add('argUniqueId', wksObj.UniqueId);
					params.add('argDocNo', docNo);
					params.add('argMsgId', msgId);
					params.add('argFileServerWS', wsUrl);
					params.add('argWorkFilename', workFilename);
					params.add('argFlags', flags);
					params.add('argOrgNo', orgNo);
					params.add('argPincode', pincode);
					params.add('argIsDraft', '0');
					params.add('argUserName', theSSO.User.account);
					wsUrl = wksObj.Url;
				}
			...
			signEnvelope : function(artifact, docNo, msgId, signValue, signCert, orgNo, options) {
				...
				//1140508	Leslie[1140331]	[退輔會]Merge[1111194]依設定決定是否改走工作站模式
				if(theSSO.User.SystemSets.get('DOCSIGN_BY_WORKSTATION') == 'Y'){
					var wksObj = theWebServices.webFileIO.ws_GetWebSfolder(wsUrl);
					wsUrl = wksObj.Url;
					wsFuncName = "wksSignEnvelope";
			...
			getCOMDocMergeStatus : function(artifact, orgNo, docNo, msgId, mainDocNo, mainDocMsgId, flags, options) {
				...
				if(theSSO.User.SystemSets.get('DOCSIGN_BY_WORKSTATION') == 'Y'){
					var wksObj = theWebServices.webFileIO.ws_GetWebSfolder(wsUrl,true);
					flags = (SSOUtil.isValueTrue(theSSO.User.EnvSettings.get('II_SUBMIT_SIGN')))?flags:Number(flags)+0x1000;		//通知wksUpdateSignFolder()執行完即主動上傳檔案
					wsFuncName = "wksGetCOMDocMergeStatus";
					params = new SOAPClientParameters();
					params.add('argArtifact', artifact);
					//params.add('argUniqueId', wksObj.UniqueId);
					params.add('argOrgNo', orgNo);
                                  ...
                                  function(rslt) {
                                    ...
                                    if (typeof rslt === 'object') {
										...
										if (rslt.m_bSuccess===true) {
											...
											//1140508	Leslie[1140331]	[退輔會]Merge[1111194]依設定決定是否改走工作站模式
											if(theSSO.User.SystemSets.get('DOCSIGN_BY_WORKSTATION') == 'Y')
												res.workStationPath = rslt.workStationPath;
			...
			updateCOMDocEnvelope : function(artifact, docNo, msgId, mainDocNo, mainDocMsgId, flags, orgNo, pincode, options) {
				...
				if(theSSO.User.SystemSets.get('DOCSIGN_BY_WORKSTATION') == 'Y'){
					...
					flags = (SSOUtil.isValueTrue(theSSO.User.EnvSettings.get('II_SUBMIT_SIGN')))?flags:Number(flags)+0x1000;		//通知wksUpdateSignFolder()執行完即主動上傳檔案
					wsFuncName = "wksUpdateCOMDocEnvelope";
					params = new SOAPClientParameters();
					params.add('argArtifact', artifact);
					params.add('argUniqueId', wksObj.UniqueId);
					params.add('argDocNo', docNo);
			...
			//1140508	Leslie[1140331]	[退輔會]Merge[1111194]新增彙併辦子文專用的SignEnvelope函式，以區分是否走工作站模式
			comDocSignEnvelope : function(artifact, docNo, msgId, signValue, signCert, orgNo, options) {
				var _dfd = $.Deferred();
				var wsFuncName = 'SignEnvelope';
				var wsUrl = (options && options.url) ? options.url : theWebServices.url('fileiows');
                if (!wsUrl || wsUrl.length===0) {
				...
				//1121025	Leslie[1111194]	依設定決定是否改走工作站模式
				if(theSSO.User.SystemSets.get('DOCSIGN_BY_WORKSTATION') == 'Y'){
					var wksObj = theWebServices.webFileIO.ws_GetWebSfolder(wsUrl);
					wsUrl = wksObj.Url;
					wsFuncName = "wksCOMDocSignEnvelope";
			...
			//1140508	Leslie[1140331]	[退輔會]Merge[1111194]新增輪循找工作站函式
			ws_GetWebSfolder : function (url, bGetNew, bRemove = true) {
				var sKey = "wksSignKey";
				if(!bGetNew && sKey in sessionStorage && sessionStorage[sKey] != undefined){
					var rtn = JSON.parse(sessionStorage[sKey]);
					if(bRemove)
					sessionStorage.removeItem(sKey);	//一般加簽均為二次行為，用完即可刪除，失敗的話就重新開始
					return rtn;
				}
				var wsFuncName = 'getWebSfolder';
				var wsUrl = (url) ? url : theWebServices.url('fileiows');
				// get WebMethodInfo
				var params = new SOAPClientParameters(), res;

				SOAPClient.invokeJSON(wsUrl, wsFuncName, params, false, function (r) {
					if (r.error == true) {	// invokeJSON回傳錯誤的物件結構需轉成呼叫invoke時的格式
						...
						if (!!r.errorDetail) {
							...
							theLogger.error('-E- FileIOWS.' + wsFuncName + ' was invoked, but WS\'s url was missing! ErrorCode=' + r);
						}
						res = 0;
					}
					else
				...
				res = res?res:0;				
				var iosConvertList = SSO_CONFIG.getIsoConvertURLs();
				var rtnUrl = iosConvertList[res]??iosConvertList[0];
				rtnUrl = rtnUrl.replace(/imgconvert/i,'WebSFolder').replace(/imgconvertws.asmx/i,'WebSFolder.asmx')
				var rtn = {'UniqueId':Util.genGUID() ,'Url':rtnUrl};
				sessionStorage.setItem(sKey,JSON.stringify(rtn));
...
function MPDocObj(rawObj) {
	...
	var _mapForUpdate = [
		...
		{ attr: 'priority', nodeName: 'PRIORITY' }
	];
	...
	function _updateWWKFCoWorkFlow(arrCoworkUnit) {
		...
		function _groupWWKFFlow(_docObj, returnToICUserTX) {
			...
			for(i=0; i<wwkf.length; i++) {
				...
				if (docOwnOUId_fc==ownOUId_fc) { // 相同一級單位項目
					...
					if (ownOUId_fc>=sso_const.VIRTUALUNIT_NUM) { // 虛擬及決行單位[95以上], 保留, 會辦單位之後
						//1140102	Joe		序1276		修正已走過的一層流程應保留在最前方--S
						if (item.SIGN_F=='Y') { // 已執行過之流程
							beforeInner.push(item);
						}
						else{
						//1140102	Joe		序1276		修正已走過的一層流程應保留在最前方--E
							after.push(item);
						}
					}
					else if (!isNaN(numOUId)) {
						if (item.SIGN_F=='Y') { // 已執行過之流程，一律保留, 會辦單位前
							//1140102	Joe		序1276		一併修正已執行過之流程應為beforeInner
							// before.push(item);
							beforeInner.push(item);
						}
						else { // 會辦單位, 刪除
		...
		function _groupWWKFFlow_CoWorkOnly(_docObj) {
			...
			for(i=0; i<wwkf.length; i++) {
				...
				if (ownOUId.length && docOwnOUId_fc!=ownOUId_fc) { // 不同一級單位項目
					...
					if (ownOUId_fc>=sso_const.VIRTUALUNIT_NUM) { // 虛擬及決行單位[95以上]
						continue;
					}
					all.push(item);
				}
		...
		function _groupNewCoworkFlow(_docObj, arrCoworkUnit) {
			...
			for(i=0; i<arrCoworkUnit.length; i++)
			{
				...
				//1141030 David 序333 因自行輸入的會辦單位會給與一組10000開始的代碼供簽稿會核單判斷使用，避免誤判新增將此類是為無代碼
				//if (unit.value===null || unit.value.length===0) { //未給單位代碼
				if (unit.value===null || unit.value.length===0 || unit.value.length > 3) {
				...
				if (typeof ownOUId=='string' && ownOUId.length>=sso_const.FIRSTCLASS_UNITNO_LEN && (ownOUId!=docOwnOUId)) { // 排除目前公文所在單位
					...
					//1141216 David 序447 會稿單位設定連動預排流程時，排除一層決行單位(會稿單位選擇一層決行單位是為了簽核框要有，不是真的要順會)
					if(ownOUId >= "95")
						continue;
			...
			if (rslt.doUpdateProcess)
			{
				...
				if (typeof wwkf_old=='object' && wwkf_old!==null)
				{
					...
					if (co_wwkf_new.Units.length) {
						//1131217	Joe		序429		修正參數數量傳錯導致無法正確判斷的問題
						// let unitItems = _makeNewOuterItem(that, co_wwkf_new.Units, rslt.syncFlow, orgNo, errItemOuter);
						let unitItems = _makeNewOuterItem(that, co_wwkf_new.Units, rslt.syncFlow, orgNo, returnToICUserTX, errItemOuter);
						if (SSOUtil.typeOf(unitItems)=='array' && unitItems.length) {
					...
					if (co_wwkf_new.AfterApp.length) {
						...
						let unitItems = _makeNewAfterAppItem(that, co_wwkf_new.AfterApp, orgNo, errItemAfterApp);
						if (SSOUtil.typeOf(unitItems)=='array' && unitItems.length) {
	...
	//1140923	Leslie[1140848]	開啟可核決公文時先記錄預排流程的傳送對象，供取消核決時設定之用
	function _setWWKFNext(_wwkfNext) {
		that.wwkfNext = _wwkfNext;
	}
	function _getWWKFNext() {
		return that.wwkfNext;
	}
	...
	//1140923	Leslie[1140848]	開啟可核決公文時先記錄預排流程的傳送對象，供取消核決時設定之用
	that.getWWKFNext = _getWWKFNext;
	that.setWWKFNext = _setWWKFNext;
...
function ToDoListBuilder() {
	...
	var _customTodo = {
		...
		fromSubject: {
			...
			Column : function(fromSubject, doc){
				...
				if (!!_debug) {
					...
					//1131121	Leslie[1130552、1130877]	修正MP主旨重覆執行編碼後，造成的顯示異常
					...
					return '<td class="subject" data-prop="subject" title="'+ theSSO.Util.htmlEncode(sfolder) + ', MsgId=' + theSSO.Util.htmlEncode(doc.msgId) + '">' + theSSO.Util.htmlEncode(sSubj) + '</td>';
				}
				else {
					...
					return `<td class="subject"  data-prop="subject" title="${theSSO.Util.htmlEncode(fromSubject)}">${theSSO.Util.htmlEncode(sSubj)}</td>`;
				}
			}
		},
		txName: {
		...
		//1140924	Leslie[1141104]	外貿協會新增「優先度」(星號)功能欄位
		priority:{
			Header : '<th data-prop="priority" style="width:1.2em"><a href="#" data-prop="priority">優<br/>先</a><div class="sortIcon"></div></th>',
			Column : function(priority, doc){
				if(doc.signType == 'W')
					return '<td></td>';
				else if(priority == '0' || priority == '')
					return '<td class="PRIORITY"></td>';
				else
					return `<td data-prop="priority" title="優先度" class="PRIORITY MP-Icon MP-PR${priority}"></td>`;
			}
		},
	}
	...
	// 2018.9.28 - 1070955
	//1140924	Leslie[1141104]	外貿協會新增「優先度」(星號)功能欄位，add showPriority
	function _buildListHeader(hideLights, showSignType, showSignDueDate, 
							...
							showCurrLoc, showDocProperty, showDocPropertyLen, showRcvDate, showPriority) {
		...
		if (!hideLights) {
			...
			sMPCol += '<th data-prop="speed" style="width:1.2em"><a href="#" data-prop="speed">速<br/>別</a><div class="sortIcon"></div></th>' +
					  '<th data-prop="light" style="width:1.2em"><a href="#" data-prop="light">燈<br/>號</a><div class="sortIcon"></div></th>' ;
			theCustom.CustomSet.CustomTodoSet = theCustom.CustomSet.CustomTodoSet.concat(['speed','light']);
			if(showPriority){
				sMPCol += '<th data-prop="priority" style="width:1.2em"><a href="#" data-prop="priority">優<br/>先</a><div class="sortIcon"></div></th>';
				theCustom.CustomSet.CustomTodoSet.push('priority');
			}
			sMPCol += '<th data-prop="secret" style="width:1.2em"><a href="#" data-prop="secret">密<br/>等</a><div class="sortIcon"></div></th>';
			theCustom.CustomSet.CustomTodoSet.push('secret');
...
// inner functions
	...
	_self._createNotifyItem_List = function(doc, sn) {
		...
		*/
		...
		//1141023	Leslie[1140849]	MP列表增加顯示選取底色功能
		if(theCustom.getCustomSet('MPListSelectedColor') !== '' && doc.msgId == _lastSelectMsgId){
			$item.css('background-color',theCustom.getCustomSet('MPListSelectedColor'));
		}
		...
		*/
		...
		if (doc.fromSubject.length) {
			...
			if (_debug) {
				...
				$subItem = $('<td class="subject" data-prop="subject" title="'+ theSSO.Util.htmlEncode(sfolder) + '">' + theSSO.Util.htmlEncode(sSubj) + '</td>');
			}
			else {
				//1131121	Leslie[1130552、1130877]	修正MP主旨重覆執行編碼後，造成的顯示異常
				// $subItem = $('<td class="subject"  data-prop="subject">' + theSSO.Util.htmlEncode(SSOUtil.escapeXml(sSubj)) + '</td>');
				$subItem = $('<td class="subject"  data-prop="subject">' + theSSO.Util.htmlEncode(sSubj) + '</td>');
			}
		}
		else {
	...
	*/
	...
	var _getDocFromODWMSG = function(docNode) {
		...
		if ((signType=='P'||signType=='E') && (!excludeDoc))
		{
			...
			doc.docProc='';	
			//1140924	Leslie[1141104]	外貿協會新增「優先度」(星號)功能欄位
			doc.priority = SSOUtil.xml_getChildNodeValue(docNode, 'PRIORITY')||'0';
		}
			...
			if (addFlowInfo || addCoworkPage) {
				...
				if (addFlowInfo) {
					...
					if (pageUrl!=='') {
						...
						urlWithParam = pageUrl + '?pDocNo=' + _docObj.docNo + '&SOURCE_ORGNO=' + _docObj.sourceOrgNo;
				...
				if (addCoworkPage) {
					...
					if (pageUrl!=='') {
						...
						urlWithParam = pageUrl + '?SOURCEORGNO=' + _docObj.sourceOrgNo + '&DOC_NO=' + _docObj.docNo;
						_menuItems['WebApp:' + pageUrl] =  {
							name: '分會進度查詢',
							'urlWithParam': urlWithParam,
						};
									...
									if (auditDocRslt.success===false) {
										...
										if (showED411===true) {
											...
											if (typeof wsEDSit=='string' && wsEDSite.length) {
												...
												urlWithParam += ('ED4/ED411.aspx?OrgNo=' + docObj.sourceOrgNo + '&DocNo=' + docObj.docNo);
	...
	_self._createNewItem_List_DOMStr = function(doc, sn, rrbGrayColorFolders, bEnableCustomTodo)  { // doc is an object
		...
		*/
		...
		if(!that.narrowWindow && bEnableCustomTodo){
			...
			let sSelectColor = '';
			if(theCustom.getCustomSet('MPListSelectedColor') !== '' && doc.msgId == _lastSelectMsgId){
				sSelectColor = ` style="background-color:${theCustom.getCustomSet('MPListSelectedColor')}"`;
			}
			let sItemTr = `<tr data-msgId="${theSSO.Util.htmlEncode(('newMsgId' in doc && doc.newMsgId.length !== 0)?doc.newMsgId:doc.msgId)}"${sItemDraft} data-light="${theSSO.Util.htmlEncode(_lightInfo.light)}" ${sSelectColor}>`
		...
		//1141023	Leslie[1140849]	MP列表增加顯示選取底色功能
		if(theCustom.getCustomSet('MPListSelectedColor') !== '' && doc.msgId == _lastSelectMsgId){
			sItemAttr += ` style="background-color:${theCustom.getCustomSet('MPListSelectedColor')}"`;
		}
		...
		if (doc.fromSubject.length) {
			...
			if (!!_debug) {
				...
				sSubItem = '<td class="subject" data-prop="subject" title="'+ theSSO.Util.htmlEncode(sfolder) + ', MsgId=' + theSSO.Util.htmlEncode(doc.msgId) + '">' + theSSO.Util.htmlEncode(sSubj) + '</td>';
			}
			else {
				//1131121	Leslie[1130552、1130877]	修正MP主旨重覆執行編碼後，造成的顯示異常
				// sSubItem = '<td class="subject"  data-prop="subject">' + theSSO.Util.htmlEncode(SSOUtil.escapeXml(sSubj)) + '</td>';
				sSubItem = '<td class="subject"  data-prop="subject">' + theSSO.Util.htmlEncode(sSubj) + '</td>';
			}
		}
		else {
	...
	*/
	 ...
	 */
		var _getDocFromODWMSG2 = function(odwmsg) {
			...
			if ((signType=='P'||signType=='E') && (!excludeDoc)) {
				...
				doc.docProc = '';
				//1140924	Leslie[1141104]	外貿協會新增「優先度」(星號)功能欄位
				doc.priority = _getValue(odwmsg, 'PRIORITY')||'0';
			}
			else if (signType=='W' && (!excludeDoc)) {
	...
	var _updateToDoListHtmlItem = function(docObj, iconCntrId, listCntrId, searchCntrId) {
		...
		if (listCntrId.length!=0) {
			if (that.listFilterFolder.length) {
				//1131101	Leslie[北榮序346]	增加支援二階層文件夾
				// if (folder===that.listFilterFolder) {
				if (folder===that.listFilterFolder || folder.split('-')[0] == that.listFilterFolder) {
					updateList = true;
				}
			}
			else {
		...
		if (searchCntrId.length!=0) {
			if (that.searchListFilterFolder.length) {
				...
				if (folder===that.searchListFilterFolder  || folder.split('-')[0] == that.searchListFilterFolder) {
	...
	function _updateDocObjByODWMSG(msgId, odwmsgNode) {
		...
		for(var i=0; i<docCnt; i++)
		{
			...
			if (docObj.msgId==msgId) {
				//1141118	Leslie[北榮急序-19]	
				_lastSelectMsgId = msgId;
	...
	function _setShowColumns(hideLights, showSignType, showSignDueDate, showICOU, showICUser, hideTransInfo, showFromOU, showFromOrg, 
							 ...
							 showDocProperty, showDocPropertyLen, showRcvDate, showPriority) 
	{
		//1111124	Leslie[1111127]	修改MP清單圖示顯示方式，並支援依客製化設定改為文字圖示
		...
		//1140924	Leslie[1141104]	外貿協會新增「優先度」(星號)功能欄位，add showPriority
		else if (!showSignType || !showICOU || !showICUser || hideTransInfo || !showFromOU || showFromOrg || showDocProperty || showRcvDate || !showSignDueDate || showPriority) {
			that.listHeader = _buildListHeader(hideLights, showSignType, showSignDueDate, showICOU, showICUser, hideTransInfo, showFromOU, showFromOrg, 
											   showCurrLoc, showDocProperty, showDocPropertyLen, showRcvDate, showPriority); // 2019.2.21 - 1080211, add showDocPropertyLen	// 2022.4.19 - 1110064, add showRcvDate
...
// Exported public methods (ToDoListBuilder)
	...
	//1141023	Leslie[1140849]	MP列表增加顯示選取底色功能
	that.getSelectMsgId = function(){
		return _lastSelectMsgId;
	}
