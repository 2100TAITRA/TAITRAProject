...
/*
Date	SA		PG		NO			Desc
1130828	David	David	1130770		勤益科大二代升級
*/

(function() {	
	theCustom.register(CUSTOM.LOAD, function(dm) 
	{
		console.log("[NCUT客製化] 載入文稿完成...");
		var dec;
		...
		.register(CUSTOM.CHANGE, "決行層次",Common.fnAppLvlModify)//新增決行層級連動處理
		.register
		(CUSTOM.REFRESH, "發文機關", 
			[	// 發文機關異動時連動以下欄位
	...
	theCustom.CustomSet = {
		ShowUnReSignCnt : true,	//新增依設定顯示是否有待補簽公文，並提供點擊後開啟IFT940
	}
	
	// 支援動態載入
    if(window.theModMgr != undefined)
        window.theModMgr.install("Custom_NCUT.js").finish();
})();