...
1140815	1141232		Raymond	Raymond	Merge[1130977], 移除MobiScroll, Merge[1130733] 配合升級Barcode第三方套件
*/
...
if(_currPage.toUpperCase() == "EDOCM.HTML"){
	...
	//1140815 Raymond 1141232 Merge[1130733]配合升級第三方套件
...
function _postLoginProcess(SAMLart, registerIP, pincode) {
	...
	function _setupLogger() {
		// 1130814 Raymond 1130313 離線模式不要
	...
	function _parseUserInfo(SAMLart, rslt) {
		...
		//1140815 Raymond 1141232 Merge[1130977]移除MobiScroll
	...
	function _parseUserInfoByJSON(SAMLart, rslt) {
		...
		if (typeof rslt === 'object') {
			...
			if (!window.theSSO.User.EnvSettings) {
				...
				// 1130809 Raymond 1130313 合併1111007(1100394), copy from getOrgInfo@RD-SysUtil.js
				...
				getOrgInfo(window.theSSO.Artifact, window.theSSO.User.orgid);
	...
	theWebServices.SAMLWS.getUserInfoForPadByJSON(SAMLart, {async:true})
		.then(function(rslt) {
			...
			// 1130809 Raymond 1130313 合併1111007(1100394), 若GetUserInfoForPadbyJSON是新帳號所產生的要預設開啟使用者資訊維護子視窗
			if("isTemp" in rslt && rslt.isTemp == true)
				window._shouldOpenUserInfoManPage = true;
		...
		.then(function() {
			...
			/* 1130809 Raymond 1130313 合併1111007(1100394), 離線版不要下載MPUiSetting.xml(因為同步)
			if (typeof SSO_CONFIG.debugTime==='boolean' && SSO_CONFIG.debugTime===true) {
				_tmBefore = Date.now();
			}
			return _getSettingRsrcFile(SAMLart);*/
		})
		/* 1130809 Raymond 1130313 合併1111007(1100394), 離線版不要載TodoList
		.then(function(){
			if (typeof SSO_CONFIG.debugTime==='boolean' && SSO_CONFIG.debugTime===true) {
		...
		/* 1130809 Raymond 1130313 合併1111007(1100394), 離線版不要載公文系統首頁
		//1111223	Leslie[1111400]	登入系統時，如有待補簽的公文，需顯示提示訊息
		// .then(function(){
		...
		.then(function() {
			...
			//if ($('#aki800ListWorkspace #querydoc_todolist_cntr .sBase').length===0) {
				...
				// 1130809 Raymond 1130313 合併1111007(1100394), 離線模式不要
				if(!theSSO || theSSO.offlineMode != true) {
			...
			/* 1130809 Raymond 1130313 合併1111007(1100394), 離線版不要連線SignalR
		...
		.then(function() {	// 2016.7.5 新增初始化樣版清單
			...
			function(tmplName, rsrcObj) {
				...
				if (!isAOLOpened) {
					// 1130809 Raymond 1130313 合併1111007(1100394), 離線版公文創稿 copy from newDocProc@RD-NewDoc.js
					...
					function _acceptInchargeOuId(OuId) {
						switch(OuId) {
						// 1130809 Raymond 1130313 合併1111007(1100394), 離線模式可創稿, 總發可用開啟舊檔方式匯入承辦人另存之ZIP檔
					...
					theSSO.MP.todolist.builder.createNewDoc(SAMLart, 'P', null, inchargeRole, docType)
					.then(function(rslt){
						if (rslt.success && !!rslt.docObj) {
							localStorage.new_draft_from_tmpl = JSON.stringify(rsrcObj);
							// 1130809 Raymond 1130313 合併1111007(1100394), 離線模式不要從todolist取得docObj, 以免因為msgId固定是100而取到已設定公文文號的前一筆創稿
							if(!!theSSO && theSSO.offlineMode == true)
								var _docObj = rslt.docObj;
							else
							...
							theSSO.MP.openDocWithAOL(localStorage.Artifact, _docObj, 'todolist', true, {aol_disable_odc010: true});	// 1100802 Raymond 1100394 離線版不顯示公文基資
					...
					// end of 1130809 Raymond 1130313 合併1111007(1100394), 離線版公文創稿 copy from newDocProc@RD-NewDoc.js
			...
			// 1130809 Raymond 1130313 合併1111007(1100394), 若GetUserInfoForPadbyJSON是新帳號所產生的要預設開啟使用者資訊維護子視窗
			if(window._shouldOpenUserInfoManPage == true) {
				theLogger.log("GetUserInfoForPadbyJSON是新帳號所產生的要預設開啟使用者資訊維護子視窗");
				setTimeout(function() {
					$("#btn_reload").trigger("click");
				}, 1000);
			}
			
			/* 1130809 Raymond 1130313 合併1111007(1100394), 離線模式不提供個人範本
		...
		.then(function() {
			/* 1130809 Raymond 1130313 合併1111007(1100394), 離線模式不提供
		...
		.fail(function(err) {
			...
			if (_href.indexOf(_EDocPage)!==-1) {
				...
				//window.location.href = SSO_CONFIG.LoginPage;	// 1130809 Raymond 1130313 合併1111007(1100394), 離線版不要跳回連線版
			}
...
function _bindTopToolBarButtons() {
	...
	$('#btn_reload, .btn_reload').on('click', function() {
		// 1130813 Raymond 1130313 合併1111007(1100394) 離線模式下, 重取待辦鈕改為使用者資訊, 點擊開啟使用者資訊子視窗
		Util.getDlg('RD-UserInfoSetting.html').done(function($dlg) {
			$.modal($dlg, {appendTo:$("#home"), containerCss:{height:450, width:500}, autoResize:true, focus:false,
				onShow: function() {
					$dlg.trigger("create");
					...
					//1100394	Leslie	增加讀入當前使用者資訊
					$dlg.find("#inpUserId").val(theUserInfo.UserID);
					$dlg.find("#inpUserNm").val(theUserInfo.UserName);
					$dlg.find("#inpTitle").val(theUserInfo.Title);
					$dlg.find("#inpEMail").val(theUserInfo.Email);
					$dlg.find("#inpTel").val(theUserInfo.Tel);
					$dlg.find("#inpTelExt").val(theUserInfo.TelExt);
					$dlg.find("#inpFax").val(theUserInfo.Fax);
					$dlg.find("#inpOrgNm").val(theUserInfo.OrgName);
					$dlg.find("#inpOrgNo").val(theUserInfo.OrgID);
					$dlg.find("#inpAddr").val(theUserInfo.OrgAddr);
					$dlg.find("#inpOrgNickName").val(theUserInfo.OrgNickName);
					$dlg.find("#inpDeptNm").val(theUserInfo.DepartName);
					$dlg.find("#inpDeptNo").val(theUserInfo.DepartID);					
					$dlg.find("#inpRoldId").val(theUserInfo.RoleID);					
					
					//1100394	Leslie	確定時，更新UserInfo內容
					$dlg.find("#ok").on("click",function(){
						//theUserInfo.UserID = $dlg.find("#inpUserId").val();
						theUserInfo.UserName = $dlg.find("#inpUserNm").val();
						theUserInfo.Title = $dlg.find("#inpTitle").val();
						theUserInfo.Email = $dlg.find("#inpEMail").val();
						theUserInfo.Tel = $dlg.find("#inpTel").val();
						theUserInfo.TelExt = $dlg.find("#inpTelExt").val();
						theUserInfo.Fax = $dlg.find("#inpFax").val();
						theUserInfo.OrgName = $dlg.find("#inpOrgNm").val();
						theUserInfo.OrgID = $dlg.find("#inpOrgNo").val();
						theUserInfo.OrgAddr = $dlg.find("#inpAddr").val();
						theUserInfo.OrgNickName = $dlg.find("#inpOrgNickName").val();
						theUserInfo.DepartName = $dlg.find("#inpDeptNm").val();
						theUserInfo.DepartID = $dlg.find("#inpDeptNo").val();
						theUserInfo.RoleID = $dlg.find("#inpRoldId").val();
						theSSO.RawUser.AD_Account.m_Name = theUserInfo.UserName;
						theSSO.RawUser.AD_Account.m_Title = theUserInfo.Title;
						theSSO.RawUser.AD_Account.m_Email = theUserInfo.Email;
						theSSO.RawUser.AD_Account.m_OfficePhone = theUserInfo.Tel;
						theSSO.RawUser.AD_Account.n_TelExt = theUserInfo.TelExt;
						theSSO.RawUser.AD_Account.m_Fax = theUserInfo.Fax;
						theSSO.RawUser.AD_Account.n_OrgName = theUserInfo.OrgName;
						theSSO.RawUser.AD_Account.m_SourceOrgNo = theUserInfo.OrgID;
						theSSO.RawUser.AD_Account.n_OrgAddr = theUserInfo.OrgAddr;
						theSSO.RawUser.AD_Account.OrgNickName = theUserInfo.OrgNickName;
						theSSO.RawUser.AD_Account.n_DepartName = theUserInfo.DepartName;
						theSSO.RawUser.AD_Account.n_DepartID = theUserInfo.DepartID;
						if('serviceWorker' in navigator) {
							navigator.serviceWorker.ready.then( registration => {
								...
								registration.active.postMessage("activeRoleIndex=" + theSSO.User.activeRoleIndex);	// 1100819 Raymond 1100394 修改RawUser後傳入SW會使activeRoleIdx重設為0, 故需將目前activeRoleIndex也傳入
							});
						}
	...
	$('#btn_logout').on( 'click', function() {
		...
		if (theSSO.logOutNow || confirmed)
		{
			...
			/* 1130809 Raymond 1130313 合併1111007(1100394), 離線模式不要呼叫logout
			...
			let loginPage = "SSO_Offline.html";		// 1130809 Raymond 1130313 合併1111007(1100394), 離線版登入頁
			...
			if (_href.indexOf(_EDocPage)!==-1 && !newLocationAssigned) {
				//let loginPage = SSO_CONFIG.LoginPage;	// 1130809 Raymond 1130313 合併1111007, 不要用連線版登入頁
				if (loginPage.length) {
...
$(function() {
	...
	SSO_CONFIG.EDocPage = "eDoc_Offline.html";	// 1130809 Raymond 1130313 合併1111007, 更改為離線版主頁
	...
	function onSSOResize(evt, ui) {
		...
		/* 1130809 Raymond 1130313 合併1111007, 離線版不需要繪製圓餅圖
		//20170414 Kevin 首頁重繪
		if(theSSO.logoned)
		{
	...
	function _openDocWithAOL(SAMLart, docObj, trigger, closeSideDrawer, extraOption) {
		...
		if (trigger=='todolist') {
			...
			if (isDraft) {
				/* 1130809 Raymond 1130313 合併1111007, 離線版草稿不要叫用ODMSSP.SetDraftMsgStatus, 直接resolve
		...
		_prm.done(function() {
			...
			*/
			if (theSSO.aolModuleLoaded) {
				...
				// 1130809 Raymond 1130313 合併1111007(1100394), 離線模式不要updateDocObj
				if(!theSSO || theSSO.offlineMode != true) {
				// 2016.12.20
    ...
    $(document).on('pagecreate', '#home', function(event, ui) {
		...
		// 1130809 Raymond 1130313 合併1111007(1100394), 偵測離線狀態及SW重置次數
		if("serviceWorker" in navigator &&
			...
			navigator.serviceWorker.addEventListener('message', function(evt) {
				...
				if(typeof evt.data == "string") {
					if(evt.data.match(/sw reinit/)) {
						$("#service_worker_reinit").text(++swReInitCnts);
					}
					else if(evt.data.match(/^offlineMode set to (\w+)/)) {
						let m = RegExp.$1;
						$("#service_worker_offlinemode").text(m);
					}
				}
			});
			navigator.serviceWorker.ready.then(function(registration) {
				...
				if(registration.installing) {
					registration.installing.postMessage("offlineMode=true");	// 若已安裝過離線版ServiceWorker, 離線模式登入頁要先通知ServiceWorker預設為離線模式
					console.log("postMessage1 - ", (new Date() - t0) + "ms");
				}
				else if(registration.waiting) {
					registration.waiting.postMessage("offlineMode=true");	// 若已安裝過離線版ServiceWorker, 離線模式登入頁要先通知ServiceWorker預設為離線模式
					console.log("postMessage1 - ", (new Date() - t0) + "ms");
				}
				else if(registration.active) {
					registration.active.postMessage("offlineMode=true");	// 若已安裝過離線版ServiceWorker, 離線模式登入頁要先通知ServiceWorker預設為離線模式
					console.log("postMessage1 - ", (new Date() - t0) + "ms");
				}
			});
		}
		...
		/* 1130809 Raymond 1130313 合併1111007(1100394), 離線版不需要智慧卡登入
		...
		// 1130809 Raymond 1130313 合併1111007, 離線版不要
    ...
    $(document).on('pageshow', '#home', function(event, ui) {
		...
		// 1130809 Raymond 1130313 合併1111007(1100394), 目前是固定離線模式, 不能任意切換
		if(!!window.theSSO)
			theSSO.offlineMode = true;
		...
		// 2021.4.19 - 1100333, Eric Peng - 分割Login(登入)/eDoc頁面!
			...
			if (_href.indexOf(_EDocPage)!==-1) {
				...
				if (typeof SAMLart=='string' && SAMLart.length) {
					...
					window.location.href = "SSO_Offline.html";	// 1130809 Raymond 1130313 合併1111007(1100394), 離線版登入頁
				}
			}
		}
