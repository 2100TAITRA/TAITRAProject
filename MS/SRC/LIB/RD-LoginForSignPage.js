...
(function($){
	if (!!theWebServices) {
		...
		theWebServices.authws = {
			...
			logout : function(artifact, options) {
                ...
                SOAPClient.invoke(wsUrl, "Logout", params, false,
                                  function(r) {
			...
			changeActiveRole : function(artifact, orgNo, unitNo, id, options){
			    ...
			    SOAPClient.invoke(wsUrl, "ChangeActiveRole", params, false,
                                  function(r) {
                                    theLogger.log(r);
                                    if(typeof r == 'boolean') {
					res = r;
	...
	theSSO.MP.loginByCert = function loginByCert(org_id, pincode, finish_callback) {
        ...
        var sc1 = new SmartCard();
		//call兩次SmartCard會有異常，先只call一次
		sc1.getCert()
        .then(function(rslt) {
            var _dfd = $.Deferred();
...
$(function() {
	function _initControls() {
		...
		if (typeof lorgid === 'string' && lorgid.length) {
			...
			$orgId[0].value = org_name;
		}
		else if (!lorgid || lorgid=="") {
			$orgId[0].value = SSO_CONFIG.getOrgInfo(0).orgName;
		}
		
		if (typeof luserid === 'string' && luserid.length) {
			$orgId[0].value = luserid;
		}
	}
    ...
    $('#btn_login').on('click', function() {
		...
		{
			if (typeof orgid === 'string' && typeof password === 'string' && orgid.length && password.length) {
				//1060329 Kevin 重複登入處理
				if(theSSO.MP.CheckUsing())
					return;
				...
				theSSO.MP.loginByCert(orgid, password, _postLoginProcess);
			}
			else {
				alert("密碼不可為空白!");
			}
	...
	$(document).on('pagecreate', '#login', function() {
		...
		// 2015.5 - 移除, 改在<SPAN> css style宣告pointer-events = none
		// 按展開icon觸發 input 之 click
		/*var $wrapper = $("#orgid_wrapper");
